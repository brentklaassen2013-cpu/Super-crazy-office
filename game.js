(() => {
  ...
  function destroyOfficeProp(prop,hitPos,dir,speed){
    ...
    try{onPolishPropDestroyed(prop,center,d,speed);}catch(_){ }
  }
  ...
  function handleBatOfficeHit(prev,tip,vel,speed){
    ...
    for(const w of breakableWindows){
      ...
      if(segmentHitsMesh(prev,tip,w,.09)){
        if(speed>=1.35){
          ...
          damageBatDurability(Math.max(.5,(speed-1)*.45),'glass');
    ...
    for(const prop of officeProps){
      ...
      if(hit){
        ...
        const mass=BABYLON.Scalar.Clamp(prop.mass||1,.5,5);damageBatDurability(Math.max(.25,(speed-1.2)*.20*mass),'prop');
        return;
      }
    }
  }
  ...
  function respawnOfficeProp(p){
    ...
    p.broken=false;p.loose=false;p.respawnTimer=0;p.cooldown=.25;p.npcHitCooldown=0;p.heldBy=null;p.thrownByNpc=false;p.playerHitCooldown=0;
  ...
  function updateOfficePhysics(dt){
    if(net.connected&&!net.isHost)return; // host-authoritative loose office props
    ...
    for(const p of officeProps){
      ...
      p.npcHitCooldown=Math.max(0,(p.npcHitCooldown||0)-dt);
      p.playerHitCooldown=Math.max(0,(p.playerHitCooldown||0)-dt);
      if(p.broken){
      ...
      // NPC-thrown office props can hurt the local player too.
      if(!net.connected && p.thrownByNpc && p.playerHitCooldown<=0 && p.vel.length()>1.25 && isInXR() && gameMode==="map" && !playerDead && !playerDowned){
        const pc=p.root.getAbsolutePosition?.()?.clone?.()||p.root.position.clone();
        const pp=playerWorldPos();
        if(BABYLON.Vector3.Distance(pc,pp)<(p.radius||.35)+.55){
          const sp=p.vel.length(),mass=BABYLON.Scalar.Clamp(p.mass||1,.5,5);
          const dmg=Math.max(3,Math.min(18,Math.round(2+sp*1.35+mass*.75)));
          hurtPlayer(dmg,pc);p.playerHitCooldown=.8;p.thrownByNpc=false;p.vel.scaleInPlace(.45);
          spawnPolishRing(pc,"impact",.85);polishAnnounce(`${String(p.type||"OBJECT").toUpperCase()} HIT YOU • -${dmg}`,1.0,"body",.95);
          playImpactSound(propSoundType(p.type),1.0);playSpatialTick(pc,.85);
        }
      }

      // Improvised VR weapon: a genuinely moving loose office prop can hurt the NPC.
      if(!net.connected && npc && !npc.dead && p.npcHitCooldown<=0 && !p.thrownByNpc && p.vel.length()>1.45){
        const pc=p.root.getAbsolutePosition?.()?.clone?.()||p.root.position.clone();
        const nc=npc.root.position.add(new BABYLON.Vector3(0,1.0,0));
        if(BABYLON.Vector3.Distance(pc,nc)<(p.radius||.35)+.72){
          const sp=p.vel.length();
          const mass=BABYLON.Scalar.Clamp(p.mass||1,.5,5);
          const prof=masterPropCombatProfile(p);
          const dmg=Math.max(2,Math.round((sp*1.7+mass*1.25)*careerDamageBonus()*masterOfficeMultiplier()*prof.damage));
          let dir=p.vel.clone();if(dir.lengthSquared()<.001)dir.set(0,.2,1);dir.normalize();
          applyDirectNpcDamage(dmg);npc.stun=Math.max(npc.stun,.16+prof.stun+Math.min(.55,sp*.055));
          npc.velocity.addInPlace(dir.scale(Math.min(3.2,.35+sp*.28)));p.npcHitCooldown=.55;p.cooldown=.18;
          registerChaos(Math.min(14,4+dmg*.35),"improvised");registerStyle("improvised",30+Math.min(45,dmg));
          addPlayerXP(Math.max(2,Math.round(dmg*.22)),"IMPROVISED");
          spawnPolishRing(pc,"impact",.9);polishAnnounce(`${String(p.type||"OFFICE").toUpperCase()} HIT • ${dmg}`,1.0,"body",.85);
          playImpactSound(p.mass>2.5?"metal":"body",.9);p.vel.scaleInPlace(.48);
          if(npc.hpValue<=0)finishNpc(pc,dir.scale(Math.max(2.2,sp)),sp);
        }
      }
  ...
  let gameState={coins:500,gems:15,kills:0,destroyed:0,blocks:0,selectedBat:"office",selectedSkin:"classic",selectedMap:"office",mode:"NORMAL",bats:{office:{count:1,level:1,xp:0}},skins:{classic:1},maps:{office:1},mapProgress:freshMapProgress(),crateHistory:[],tradeHistory:[],pendingDuplicate:null,duplicateQueue:[],dailyDate:todayKey(),daily:{kill:{goal:3,progress:0,coins:80,gems:1,claimed:false},destroy:{goal:8,progress:0,coins:70,gems:1,claimed:false},block:{goal:4,progress:0,coins:70,gems:1,claimed:false}},weeklyDate:weekKey(),weekly:{boss:{goal:3,progress:0,gems:12,claimed:false}},settings:{
    ...
    playerProgress:{level:1,xp:0,prestige:0,bestStyle:0,totalObjectives:0,totalMayhem:0,bossStreak:0,bestBossStreak:0,perfectBlocks:0,chainReactions:0},
    achievements:{}
  };
  ...
  // Account progression is deliberately separate from bat levels: the bat is gear mastery,
  // player level is long-term Crazy Office progression across every map and mode.
  const playerXpNeeded=l=>Math.round(90+Math.pow(Math.max(1,l),1.22)*42);
  function addPlayerXP(amount,reason="PLAY"){
    const pr=gameState.playerProgress||(gameState.playerProgress={level:1,xp:0,prestige:0,bestStyle:0,totalObjectives:0,totalMayhem:0});
    pr.xp+=Math.max(0,Math.round(amount));
    let leveled=false;
    while(pr.level<100&&pr.xp>=playerXpNeeded(pr.level)){
      pr.xp-=playerXpNeeded(pr.level);pr.level++;leveled=true;
      gameState.coins+=75+pr.level*5;
      if(pr.level%5===0)gameState.gems+=2;
    }
    if(leveled&&typeof mapEventText!=="undefined"){
      mapEventText=`PLAYER LEVEL ${pr.level} • BONUS REWARD`;mapEventTextTimer=3.2;
      playImpactSound("metal",.72);
    }
  }
  function unlockAchievement(id,label,coins=100,gems=1){
    gameState.achievements=gameState.achievements||{};
    if(gameState.achievements[id])return false;
    gameState.achievements[id]={at:Date.now(),label};gameState.coins+=coins;gameState.gems+=gems;
    if(typeof mapEventText!=="undefined"){mapEventText=`ACHIEVEMENT • ${label}`;mapEventTextTimer=3.3;}
    playImpactSound("metal",.8);saveGame();return true;
  }
  function evaluateAchievements(){
    if(gameState.kills>=1)unlockAchievement("first_ko","FIRST DAY PROBLEM",80,1);
    if(gameState.destroyed>=50)unlockAchievement("wrecking_ball","OFFICE WRECKER",180,2);
    if(gameState.blocks>=25)unlockAchievement("not_today","NOT TODAY, HR",160,2);
    if((gameState.playerProgress?.totalMayhem||0)>=5)unlockAchievement("mayhem_regular","CHAOS PROFESSIONAL",250,3);
    if((gameState.collectionStats?.bosses||0)>=3)unlockAchievement("boss_issues","MANAGEMENT PROBLEM",250,3);
    if((gameState.playerProgress?.perfectBlocks||0)>=20)unlockAchievement("perfect_20","UNION REPRESENTATIVE",180,2);
    if((gameState.playerProgress?.chainReactions||0)>=15)unlockAchievement("chain_15","WORKPLACE HAZARD",200,2);
    if((gameState.playerProgress?.bestBossStreak||0)>=5)unlockAchievement("boss_streak_5","CEO OF VIOLENCE",350,5);
  }
  ...
  function updateStatsUI(){const e=document.getElementById("gameStats");if(!e)return;const b=selectedBatData(),s=selectedBatState(),p=gameState.mapProgress[gameState.selectedMap]||{level:1};e.innerHTML=`<b>Player Lv.${gameState.playerProgress?.level||1} • ${b.name} Lv.${s.level}</b><br>🪙 ${gameState.coins} &nbsp; 💎 ${gameState.gems}<br>${gameState.selectedMap.toUpperCase()} • MAP LV ${p.level}/10 • ${gameState.mode}<br>KOs ${gameState.kills} &nbsp; Broken ${gameState.destroyed}`;}
  ...
  function loadGame(){try{const o=JSON.parse(localStorage.getItem("vrBatBrawl_v018")||"null");if(o){gameState.coins=Math.max(gameState.coins,o.coins||0);gameState.kills=o.kills||0;gameState.destroyed=o.destroyed||0;gameState.blocks=o.blocks||0;}const r=JSON.parse(localStorage.getItem(SAVE_KEY)||"null");if(r){if(!r.potatoRGB&&r.potatoColor)legacyPotatoColorToMigrate=r.potatoColor;gameState={...gameState,...r,bats:{...gameState.bats,...(r.bats||{})},skins:{...gameState.skins,...(r.skins||{})},maps:{...gameState.maps,...(r.maps||{})},mapProgress:{...freshMapProgress(),...(r.mapProgress||{})},daily:{...gameState.daily,...(r.daily||{})},weekly:{...gameState.weekly,...(r.weekly||{})},duplicateQueue:Array.isArray(r.duplicateQueue)?r.duplicateQueue:[],settings:{...gameState.settings,...(r.settings||{})},
      ...
      playerProgress:{...gameState.playerProgress,...(r.playerProgress||{})},
      achievements:{...gameState.achievements,...(r.achievements||{})}
  ...
  function recordKill(){runStats.kills++;gameState.kills++;onModeKillReward();masterRegisterKo();addPlayerXP(34,"KO");gameState.coins+=Math.round(18*chaosRewardMultiplier());chaosRewardBank+=mayhemTimer>0?4:0;registerChaos(24,"ko");registerStyle("ko",55);addBatXP(26);const p=currentMapProgress();p.waveKills++;if(p.level>=10){if(gameState.mode==="HARDCORE")cashHardcoreBank();p.bossWins++;gameState.weekly.boss.progress=Math.min(3,gameState.weekly.boss.progress+1);gameState.coins+=gameState.mode==="HARDCORE"?330:gameState.mode==="ENDLESS"?290:220;gameState.gems+=gameState.mode==="HARDCORE"?7:gameState.mode==="ENDLESS"?6:4;const bossBat=BAT_CATALOG.find(b=>b.boss===gameState.selectedMap);if(bossBat&&Math.random()<.12)awardItem("bat",bossBat.id,false);lastLobbyMessage=endRunSummary(true);gameState.collectionStats.bosses++;
      gameState.playerProgress.bossStreak=(gameState.playerProgress.bossStreak||0)+1;
      gameState.playerProgress.bestBossStreak=Math.max(gameState.playerProgress.bestBossStreak||0,gameState.playerProgress.bossStreak);
      try{onPolishBossVictory();}catch(_){ }
      p.level=gameState.mode==="ENDLESS"?10:1;p.waveKills=0;setMusicMode("normal");}else if(p.waveKills>=3){p.waveKills=0;p.level=Math.min(10,p.level+1);gameState.coins+=gameState.mode==="HARDCORE"?70:gameState.mode==="SURVIVAL"?55:45;if(p.level===10)gameState.gems+=2;}if(gameState.weekly.boss.progress>=3&&!gameState.weekly.boss.claimed){gameState.weekly.boss.claimed=true;gameState.gems+=gameState.weekly.boss.gems;}progressDaily("kill");evaluateAchievements();saveGame();}
  function recordDestruction(){gameState.destroyed++;addPlayerXP(5,"BREAK");gameState.coins+=Math.max(3,Math.round(3*chaosRewardMultiplier()));chaosRewardBank+=mayhemTimer>0?1:0;registerChaos(8,"destroy");registerStyle("destroy",15);progressDaily("destroy");evaluateAchievements();saveGame();}
  function recordBlock(){runStats.blocks++;gameState.blocks++;addPlayerXP(9,"BLOCK");gameState.coins+=Math.max(1,Math.round(chaosRewardMultiplier()));registerChaos(5,"block");registerStyle("block",34);addBatXP(2);progressDaily("block");evaluateAchievements();saveGame();}
...
Grip = back`;
  ...
  function handleHostMessage(pid,d){
    ...
    }else if(d.t==="propGrabReq"){
      masterHostTryGrabProp(pid,d);
    }else if(d.t==="propRelease"){
      masterHostReleaseProp(pid,d);
  ...
  function handleGuestMessage(d){
    ...
    if(d.t==="duplicate"){
      ...
      if(gameMode==="map")masterApplyPropState(d);
...
Grip = back`;
  ...
  function netNpcPacket(){
    ...
    return {
      ...
      emotion:npc.emotion||"normal",screamKind:npc.screamKind||null,
      voiceGender:npc.voiceGender||"male",voiceCategory:npc.voiceCategory||null,
      voiceClipId:npc.voiceClipId||null,voiceSeq:npc.voiceSeq||0,
      targetId:npc.dead?null:(target?.id||null),
  ...
  function applyHostNpcState(d){
    ...
    if(d.voiceGender==="male"||d.voiceGender==="female")npc.voiceGender=d.voiceGender;
    if(typeof d.voiceCategory==="string")npc.voiceCategory=d.voiceCategory;
    if(typeof d.voiceClipId==="string")npc.voiceClipId=d.voiceClipId;
    if(Number.isFinite(d.voiceSeq)&&d.voiceSeq>(npc.lastRemoteVoiceSeq||0)){
      npc.lastRemoteVoiceSeq=d.voiceSeq;
      if(typeof d.voiceClipId==="string"){
        tryPlayHumanVoiceClip(d.voiceClipId,d.screamKind==="death");
        npc.faceVoicePulse=1;
      }
    }
    ...
    if(net.reviveCooldown<=0&&btn(hands.left,1)&&hands.left?.node){
      ...
      for(const [pid,p] of net.players){
        ...
        if(rh&&BABYLON.Vector3.Distance(lp,rh)<1.1){
          ...
          if(gameMode==="map"){
            addPlayerXP(20,"REVIVE");gameState.coins+=10;registerStyle("revive",60);registerChaos(8,"revive");
            polishAnnounce("TEAMMATE REVIVED • +10 COINS",1.6,"perfect",.75);saveGame();
          }
          break;
        }
      }
...
hidePack2();resetBatDurability();const wasHolstered=batHolstered;batHolstered=false;if(wasHolstered&&!attachBatToSelectedHand()){batRoot.parent=null;batRoot.setEnabled(false);}mapEventText="";mapEventTextTimer=0;quickMenuDebounce=0;groundSlamCooldown=0;combatInputPrev.trigger=false;combatInputPrev.grip=false;chargeTime=0;batTipLast=null;batBaseLast=null;if(batThrown&&!attachBatToRightHand()){batRoot.parent=null;batRoot.setEnabled(false);}gameMode="map";
  ...
  function updatePlayerHud() {
    ...
    const chaosHud=gameMode==="map"&&chaosTier>0?`${chaosTierName()} ${Math.round(chaosMeter)}%${chaosCombo>1?` • x${chaosCombo} COMBO`:""}`:"";
    const objectiveHud=gameMode==="map"?runObjectiveHud():"";
    const styleHud=gameMode==="map"&&styleScore>0?`STYLE ${styleRank()} • ${styleScore}${styleChain>1?` • x${styleChain}`:""}`:"";
    const heldHud=(typeof heldOfficeProp!=="undefined")?(heldOfficeProp.left||heldOfficeProp.right):null;
    const batHud=(typeof batDurability!=="undefined")?(batBroken?`BAT BROKEN • ${Math.max(0,batReissueTimer).toFixed(1)}s`:`BAT ${batDurabilityPct()}%`):"";
    const grabHud=heldHud?`GRABBED ${String(heldHud.p?.type||"OBJECT").toUpperCase()} • RELEASE GRIP TO THROW`:"";
    const bossHud=gameMode==="map"&&currentMapProgress().level>=10&&npc&&!npc.dead?`BOSS PHASE ${npc.bossPhase||1}${npc.bossArmor>0?` • ARMOR ${Math.ceil(npc.bossArmor)}`:""}`:"";
    const masterHud=(masterComboCount>=2?`OFFICE COMBO x${masterComboCount} • `:"")+masterPromotionLabel();
    missionText.text=mapEventTextTimer>0?`${mapEventText}`:(grabHud||bossHud||chaosHud||objectiveHud||styleHud||batHud||masterHud||modeHud()||`${waveModifier!=="NONE"?"MOD: "+waveModifier:""}`);
  ...
  function hurtPlayer(amount, fromWorldPos) {
    ...
    amount=Math.max(1,Math.round(amount*masterIncomingMultiplier()));
  ...
  // Per-run bat durability. Inventory bats are never permanently destroyed.
  // Heavy hits/blocks build stress, visible cracks warn the player, then the bat can snap.
  let batDurability=100,batBroken=false,batReissueTimer=0,batCrackStage=0;
  const batCrackMeshes=[];
  for(let i=0;i<3;i++){
    const c=BABYLON.MeshBuilder.CreateTorus("batCrack"+i,{diameter:.116-i*.004,thickness:.008,tessellation:14},scene);
    c.parent=batRoot;c.rotation.x=Math.PI/2;c.position.z=.24+i*.19;c.material=batDark;c.setEnabled(false);batCrackMeshes.push(c);
  }
  function batMaxDurability(){
    const b=selectedBatData(),lv=selectedBatState().level||1;
    const rarity={Common:0,Uncommon:6,Rare:12,Epic:18,Legendary:26,Mythic:34}[b.rarity]||0;
    return 100+rarity+Math.min(24,lv*.6);
  }
  function resetBatDurability(){
    batDurability=batMaxDurability();batBroken=false;batReissueTimer=0;batCrackStage=0;
    batCrackMeshes.forEach(m=>m.setEnabled(false));
    if(batRoot.parent||gameMode==="map")batRoot.setEnabled(true);
  }
  function batDurabilityPct(){return Math.max(0,Math.round(100*batDurability/Math.max(1,batMaxDurability())));}
  function damageBatDurability(stress,reason="impact"){
    if(batBroken||gameMode!=="map"||stress<=0)return;
    stress*=Math.max(.55,1-(masterRunBuffs?.focus||0)*.09);
    const oldPct=batDurabilityPct();batDurability=Math.max(0,batDurability-stress);const pct=batDurabilityPct();
    const stage=pct<=20?3:pct<=45?2:pct<=70?1:0;
    if(stage>batCrackStage){
      batCrackStage=stage;for(let i=0;i<batCrackMeshes.length;i++)batCrackMeshes[i].setEnabled(i<stage);
      playImpactSound('crack',.72+stage*.12);pulse(batHand(),.45+stage*.12,55+stage*16);
      polishAnnounce(stage>=3?`BAT CRITICAL • ${pct}%`:`BAT CRACKED • ${pct}%`,1.05,'crack',.8);
    }
    if(batDurability<=0)breakPlayerBat(reason);
  }
  function breakPlayerBat(reason="impact"){
    if(batBroken)return;batBroken=true;batDurability=0;batReissueTimer=4.5;batThrown=false;batRecalling=false;batHolstered=false;
    const breakPos=batTip();
    const shard=BABYLON.MeshBuilder.CreateCylinder("brokenBatShard"+performance.now(),{height:.42,diameterTop:.11,diameterBottom:.075,tessellation:12},scene);
    shard.position.copyFrom(breakPos);shard.rotation.z=Math.random()*Math.PI;shard.material=batWood;
    const v=new BABYLON.Vector3((Math.random()-.5)*2.5,2.0+Math.random(),(Math.random()-.5)*2.5);let life=2.8;
    scene.onBeforeRenderObservable.add(function tick(){const dt=Math.min(.04,engine.getDeltaTime()/1000);life-=dt;v.y-=8.8*dt;shard.position.addInPlace(v.scale(dt));shard.rotation.x+=dt*7;shard.rotation.z+=dt*5;if(life<=0){scene.onBeforeRenderObservable.removeCallback(tick);shard.dispose();}});
    batRoot.setEnabled(false);playImpactSound('break',1.2);pulse(batHand(),1,150);registerChaos(10,'batBreak');
    polishAnnounce('BAT BROKE! • USE THE OFFICE',1.8,'break',1.15);
  }
  function updateBatDurability(dt){
    if(!batBroken)return;batReissueTimer-=dt;if(batReissueTimer>0)return;
    resetBatDurability();attachBatToSelectedHand();playImpactSound('pickup',.9);pulse(batHand(),.55,70);polishAnnounce('REPLACEMENT BAT',1.0,'pickup',.7);
  }
  function batTip() {
  ...
  function setMusicMode(mode){
    const next=mode==="boss"?"boss":mode==="mayhem"?"mayhem":"normal";
  ...
  function startBackgroundMusic(){
    ...
    const mayhem=[110,164.81,196,146.83,123.47,196,220,164.81];
    const tick=()=>{
      if(!musicRunning)return;
      const isBoss=musicMode==="boss";
      const isMayhem=musicMode==="mayhem";
      const seq=isBoss?boss:isMayhem?mayhem:normal;
      ...
      if(isBoss){
        ...
        playMusicNote(f,.18,.058,"square");
        if(musicStep%2===0)playMusicNote(f/2,.26,.036,"triangle");
        if(musicStep%4===3)playMusicNote(f*2,.08,.018,"sine");
      }else{
        playMusicNote(f,.28,.05,"triangle");
        if(musicStep%4===0)playMusicNote(f/2,.42,.028,"sine");
      }
      musicStep++;
      musicTimer=setTimeout(tick,isBoss?300:isMayhem?345:430);
    };
    tick();
  }
  ...
  function playImpactSound(kind="body",strength=.5){
    ...
    try{
      ...
      }else if(kind==="crack" || kind==="break"){
        const src=ac.createBufferSource(),bp=ac.createBiquadFilter(),g=ac.createGain();
        src.buffer=noiseBuffer(ac,kind==="break"?.24:.11);bp.type="bandpass";bp.frequency.value=kind==="break"?520:760;bp.Q.value=.72;
        g.gain.setValueAtTime((kind==="break"?.16:.09)*strength,now);g.gain.exponentialRampToValueAtTime(.001,now+(kind==="break"?.24:.11));
        src.connect(bp);bp.connect(g);g.connect(ac.destination);src.start(now);
        const o=ac.createOscillator(),og=ac.createGain();o.type="triangle";o.frequency.setValueAtTime(kind==="break"?150:260,now);o.frequency.exponentialRampToValueAtTime(kind==="break"?70:135,now+.13);
        og.gain.setValueAtTime(.055*strength,now);og.gain.exponentialRampToValueAtTime(.001,now+.16);o.connect(og);og.connect(ac.destination);o.start(now);o.stop(now+.18);
      }else if(kind==="pickup"){
        const o=ac.createOscillator(),g=ac.createGain();o.type="sine";o.frequency.setValueAtTime(330,now);o.frequency.exponentialRampToValueAtTime(520,now+.055);
        g.gain.setValueAtTime(.035*strength,now);g.gain.exponentialRampToValueAtTime(.001,now+.08);o.connect(g);g.connect(ac.destination);o.start(now);o.stop(now+.09);
      }else if(kind==="slap"){
  ...
  // ------------------------------------------------------------
  // ------------------------------------------------------------
  // Crazy Office NPC reaction voices — embedded for the 2-file build.
  // Male + female sets. No spoken dialogue / no TTS.
  // Grunts are common; full screams are intentionally rare.
  // ------------------------------------------------------------
  const HUMAN_VOICE_FILES_AVAILABLE=true;

  const NPC_VOICE_DATA={
    "m_grunt1":"data:audio/mpeg;base64,",
    "m_grunt2":"data:audio/mpeg;base64,",
    "m_hurt1":"data:audio/mpeg;base64,",
    "m_hurt2":"data:audio/mpeg;base64,",
    "m_hard":"data:audio/mpeg;base64,",
    "m_gasp":"data:audio/mpeg;base64,",
    "m_panic":"data:audio/mpeg;base64,",
    "m_attack1":"data:audio/mpeg;base64,",
    "m_attack2":"data:audio/mpeg;base64,",
    "m_ko1":"data:audio/mpeg;base64,",
    "m_ko2":"data:audio/mpeg;base64,",
    "m_extreme":"data:audio/mpeg;base64,",
    "f_grunt1":"data:audio/mpeg;base64,",
    "f_grunt2":"data:audio/mpeg;base64,",
    "f_weirdgrunt":"data:audio/mpeg;base64,",
    "f_hurt1":"data:audio/mpeg;base64,",
    "f_hurt2":"data:audio/mpeg;base64,",
    "f_hard":"data:audio/mpeg;base64,",
    "f_gasp":"data:audio/mpeg;base64,",
    "f_panic":"data:audio/mpeg;base64,",
    "f_attack1":"data:audio/mpeg;base64,",
    "f_attack2":"data:audio/mpeg;base64,",
    "f_ko1":"data:audio/mpeg;base64,",
    "f_ko2":"data:audio/mpeg;base64,"
  };

  const NPC_VOICE_CLIPS={
    male:{
      grunt:["m_grunt1","m_grunt2"],
      hurt:["m_hurt1","m_hurt2"],
      hard:["m_hard"],
      gasp:["m_gasp"],
      panic:["m_panic"],
      attack:["m_attack1","m_attack2"],
      ko:["m_ko1","m_ko2"],
      extreme:["m_extreme"]
    },
    female:{
      grunt:["f_grunt1","f_grunt2","f_weirdgrunt"],
      hurt:["f_hurt1","f_hurt2"],
      hard:["f_hard"],
      gasp:["f_gasp"],
      panic:["f_panic"],
      attack:["f_attack1","f_attack2"],
      ko:["f_ko1","f_ko2"],
      extreme:["f_ko2"]
    }
  };
  ...
  function pickNpcVoiceClip(gender,category){
    ...
    const list=NPC_VOICE_CLIPS[g]?.[category]||NPC_VOICE_CLIPS[g]?.grunt||[];
    if(!list.length)return null;
    if(list.length===1)return list[0];
    const options=list.filter(x=>x!==npcVoiceLastId);
    const pool=options.length?options:list;
    return pool[Math.floor(Math.random()*pool.length)];
  }

  function tryPlayHumanVoiceClip(clipId,force=false){
    if(!HUMAN_VOICE_FILES_AVAILABLE||!clipId)return false;
    const src=NPC_VOICE_DATA[clipId];
    if(!src)return false;
    try{
      if(npcVoiceAudio&&!npcVoiceAudio.paused){
  ...
  function npcVoiceCategory(kind){
    const r=Math.random();
    if(kind==="death"){
      if(r<.05)return "extreme";
      if(r<.78)return "ko";
      return "hard";
    }
    if(kind==="attack"){
      return r<.90?"attack":"grunt";
    }
    if(kind==="hurt"){
      if(r<.58)return "grunt";
      if(r<.96)return "hurt";
      return "hard";
    }
    if(kind==="block"){
      return r<.70?"grunt":"gasp";
    }
    if(kind==="chase"){
      return r<.72?"grunt":r<.94?"gasp":"panic";
    }
    if(kind==="scared"){
      return r<.58?"gasp":r<.94?"panic":"hard";
    }
    if(kind==="furious"){
      return r<.64?"attack":r<.96?"hard":"extreme";
    }
    if(kind==="angry"){
      return r<.70?"grunt":"attack";
    }
    return "grunt";
  }
  ...
  function speakNpc(kind,force=false) {
    ...
    // Intentionally restrained: NPCs react, but do not scream on every hit.
    ...
    if(Math.random()<=chance){
      const category=npcVoiceCategory(kind);
      const clipId=pickNpcVoiceClip(npc.voiceGender,category);
      if(clipId && tryPlayHumanVoiceClip(clipId,kind==="death")){
        npc.voiceCategory=category;
        npc.voiceClipId=clipId;
        npc.voiceSeq=(npc.voiceSeq||0)+1;
        npc.faceVoicePulse=1;
      }
    }

    npc.reactionCooldown=kind==="death"?0:2.8+Math.random()*2.7;
  }
  ...
  const NPC_BEHAVIORS=[
    {id:"brawler",label:"BRAWLER",speed:1.05,attack:1.10,throw:0.72,hesitate:.92,preferred:1.55},
    {id:"coworker",label:"PANICKED",speed:1.12,attack:.82,throw:1.08,hesitate:1.34,preferred:1.85},
    {id:"thrower",label:"THROWER",speed:.92,attack:.92,throw:1.75,hesitate:1.05,preferred:2.55},
    {id:"duelist",label:"DUELIST",speed:1.00,attack:1.04,throw:.58,hesitate:.82,preferred:1.95},
    {id:"hothead",label:"HOTHEAD",speed:1.13,attack:1.16,throw:1.20,hesitate:.72,preferred:1.48}
  ];
  function chooseNpcBehavior(archetype){
    const weighted=archetype?.name==="Manager"?[2,1,2,3]:archetype?.name==="Runner"?[1,1,3,4]:archetype?.name==="Tank"?[0,0,4,3]:[0,1,2,3,4];
    return {...NPC_BEHAVIORS[weighted[Math.floor(Math.random()*weighted.length)]]};
  }
  ...
  function updateNpcFace(dt){
    ...
    if(npc.emotionBurst<=0 && npc.hpValue>0 && !(net.connected&&!net.isHost)){
      if(npc.hpValue<npc.maxHp*.22)reactNpc("lowHp",1);
      else npc.emotion=npc.archetype?.preferredEmotion||"normal";
    }
    ...
    const target=(faceTarget?.pos||(net.connected&&!net.isHost
      ? npc.root.position.add(new BABYLON.Vector3(0,1.55,1))
      : playerWorldPos())).clone();
    ...
    npc.leftPupil.position.y=.070+py;
    npc.rightPupil.position.y=.070+py;
    ...
    const pulse=npc.faceVoicePulse||0;
    const voice=npc.voiceCategory||null;

    let targetFace={eyeY:1.02,eyeX:1.03,mouthX:1,mouthY:1,browL:-.08,browR:.08};

    if(npc.emotion==="angry"){
      targetFace={eyeY:.82,eyeX:1.08,mouthX:1.22,mouthY:1.65,browL:-.42,browR:.42};
    }else if(npc.emotion==="scared"){
      targetFace={eyeY:1.24,eyeX:1.14,mouthX:.82,mouthY:3.65,browL:.22,browR:-.22};
    }else if(npc.emotion==="shocked"){
      targetFace={eyeY:1.32,eyeX:1.18,mouthX:.92,mouthY:3.95,browL:.10,browR:-.10};
    }else if(npc.emotion==="hurt"){
      targetFace={eyeY:.58,eyeX:.95,mouthX:1.10,mouthY:2.25,browL:-.18,browR:.30};
    }

    if(voice==="grunt"){
      targetFace={eyeY:.78,eyeX:1.00,mouthX:1.18,mouthY:1.48+.38*pulse,browL:-.18,browR:.20};
    }else if(voice==="hurt"){
      targetFace={eyeY:.50,eyeX:.94,mouthX:1.08,mouthY:2.25+.70*pulse,browL:-.24,browR:.32};
    }else if(voice==="hard"){
      targetFace={eyeY:1.15,eyeX:1.16,mouthX:.90,mouthY:3.55+1.05*pulse,browL:.14,browR:-.14};
    }else if(voice==="gasp"){
      targetFace={eyeY:1.38,eyeX:1.20,mouthX:.84,mouthY:3.25+.85*pulse,browL:.22,browR:-.22};
    }else if(voice==="panic"){
      targetFace={eyeY:1.43,eyeX:1.22,mouthX:.80,mouthY:4.05+1.15*pulse,browL:.30,browR:-.30};
    }else if(voice==="attack"){
      targetFace={eyeY:.66,eyeX:1.10,mouthX:1.32,mouthY:1.62+.58*pulse,browL:-.50,browR:.50};
    }else if(voice==="ko"){
      targetFace={eyeY:.30,eyeX:.98,mouthX:.98,mouthY:2.05+.40*pulse,browL:-.05,browR:.08};
    }else if(voice==="extreme"){
      targetFace={eyeY:1.48,eyeX:1.25,mouthX:.78,mouthY:4.55+1.35*pulse,browL:.34,browR:-.34};
    }

    const koTarget=(npc.dead||npc.hpValue<=0)?1:0;
    npc.faceKoBlend=BABYLON.Scalar.Lerp(npc.faceKoBlend||0,koTarget,1-Math.exp(-dt*5));
    if(npc.faceKoBlend>.01){
      const k=npc.faceKoBlend;
      targetFace.eyeY=BABYLON.Scalar.Lerp(targetFace.eyeY,.24,k);
      targetFace.eyeX=BABYLON.Scalar.Lerp(targetFace.eyeX,.96,k);
      targetFace.mouthX=BABYLON.Scalar.Lerp(targetFace.mouthX,1.04,k);
      targetFace.mouthY=BABYLON.Scalar.Lerp(targetFace.mouthY,1.55,k);
      targetFace.browL=BABYLON.Scalar.Lerp(targetFace.browL,-.02,k);
      targetFace.browR=BABYLON.Scalar.Lerp(targetFace.browR,.02,k);
    }

    const blend=npc.faceExpressionBlend||(npc.faceExpressionBlend={
      eyeY:1.02,eyeX:1.03,mouthX:1,mouthY:1,browL:-.08,browR:.08
    });
    const speed=voice?12:7;
    const alpha=1-Math.exp(-dt*speed);

    blend.eyeY=BABYLON.Scalar.Lerp(blend.eyeY,targetFace.eyeY,alpha);
    blend.eyeX=BABYLON.Scalar.Lerp(blend.eyeX,targetFace.eyeX,alpha);
    blend.mouthX=BABYLON.Scalar.Lerp(blend.mouthX,targetFace.mouthX,alpha);
    blend.mouthY=BABYLON.Scalar.Lerp(blend.mouthY,targetFace.mouthY,alpha);
    blend.browL=BABYLON.Scalar.Lerp(blend.browL,targetFace.browL,alpha);
    blend.browR=BABYLON.Scalar.Lerp(blend.browR,targetFace.browR,alpha);

    const eyeY=blinking?.10:blend.eyeY;
    npc.leftEyeWhite.scaling.x=blend.eyeX;
    npc.rightEyeWhite.scaling.x=blend.eyeX;
    ...
    npc.eyebrowL.rotation.z=blend.browL;
    npc.eyebrowR.rotation.z=blend.browR;
    npc.mouth.scaling.x=blend.mouthX;
    npc.mouth.scaling.y=blend.mouthY;

    npc.teeth.setEnabled(
      voice==="panic"||voice==="hard"||voice==="extreme"||
      npc.emotion==="scared"||npc.emotion==="shocked"||npc.emotion==="angry"
    );
  }
  ...
  function createNpc() {
    ...
    const behavior=chooseNpcBehavior(archetype);
    ...
    npc={
      ...
      emotionBurst:0,
      screamKind:null,
      voiceGender:Math.random()<.5?"female":"male",
      voiceCategory:null,
      voiceClipId:null,
      voiceSeq:0,
      lastRemoteVoiceSeq:0,
      ...
      faceBlinkAmount:0,
      faceExpressionBlend:{eyeY:1.02,eyeX:1.03,mouthX:1,mouthY:1,browL:-.08,browR:.08},
      faceVoicePulse:0,
      faceKoBlend:0,
      lastStepIndex:-1,
      recentlyHit:0,
  ...
  function configureNpcForCurrentLevel(){
    ...
    const partyN=Math.max(1,Math.min(4,gameState.partySize||1));
    const coopHpMul=1+(partyN-1)*.38;
    const coopAttackMul=1+(partyN-1)*.08;
    if(boss){
      const bossNames={office:"THE CEO",school:"THE PRINCIPAL",house:"THE LANDLORD",supermarket:"THE MANAGER",gym:"THE COACH",hotel:"THE CONCIERGE",bank:"THE DIRECTOR",metro:"THE INSPECTOR",factory:"THE FOREMAN",cinema:"THE PRODUCER",arcade:"THE CHAMPION",city:"THE ENFORCER",forest:"THE RANGER",beach:"THE LIFEGUARD",construction:"THE SITE BOSS",mall:"THE FLOOR MANAGER",police:"THE CAPTAIN",hospital:"THE CHIEF",lab:"THE DIRECTOR",stadium:"THE COACH",castle:"THE WARDEN",farm:"THE FOREMAN",pirate:"THE CAPTAIN",amusement:"THE OPERATOR",volcano:"THE OVERSEER",space:"THE COMMANDER",alien:"THE OVERLORD"};
      npc.typeName=bossNames[gameState.selectedMap]||"THE BOSS";
      npc.maxHp=Math.round(520*modeMul*coopHpMul);npc.hpValue=npc.maxHp;
      npc.archetype={...(npc.archetype||{}),speed:1.28*speedMul,attackMul:1.42*modeMul*coopAttackMul,knockbackMul:1.35};
      ...
      const scale=(1+(p.level-1)*.045)*modeMul*coopHpMul;
      npc.maxHp=Math.round(npc.maxHp*scale);npc.hpValue=npc.maxHp;
      npc.archetype={...(npc.archetype||{}),speed:(npc.archetype?.speed||1.2)*speedMul,attackMul:(npc.archetype?.attackMul||1)*modeMul*coopAttackMul};
  ...
  function updateNpcLabel() {
    ...
    npc.hp.text.text=`${npc.variant&&npc.variant!=="NORMAL"?npc.variant+" ":""}${npc.typeName||"NPC"}${currentMapProgress().level<10&&npc.behavior?.label?` • ${npc.behavior.label}`:""} • ${hp}/${max}${npc.bossArmor>0?` • ARMOR ${Math.ceil(npc.bossArmor)}`:""}`;
  ...
  // ------------------------------------------------------------
  // OFFICE MAYHEM DIRECTOR — Gauntlet 02
  // Turns combat + destruction + blocks into one readable flow loop.
  // Movement is intentionally untouched in this build.
  // ------------------------------------------------------------
  let chaosMeter=0;
  let chaosCombo=0;
  let chaosComboTimer=0;
  let chaosTier=0;
  let mayhemTimer=0;
  let chaosRewardBank=0;
  let chaosLastTierAnnounced=0;

  function chaosRewardMultiplier(){
    if(mayhemTimer>0)return 2;
    if(chaosTier>=3)return 1.5;
    if(chaosTier>=2)return 1.25;
    return 1;
  }

  function chaosTierName(){
    return chaosTier===4?"TOTAL MAYHEM":chaosTier===3?"OFFICE RIOT":chaosTier===2?"CHAOS":chaosTier===1?"HEATING UP":"";
  }

  function registerChaos(amount,kind="action"){
    if(gameMode!=="map"||amount<=0)return;
    const before=chaosTier;
    chaosMeter=BABYLON.Scalar.Clamp(chaosMeter+amount,0,100);
    chaosCombo=Math.min(99,chaosCombo+1);
    chaosComboTimer=2.7;
    chaosTier=chaosMeter>=100?4:chaosMeter>=72?3:chaosMeter>=42?2:chaosMeter>=18?1:0;

    if(chaosTier>before && chaosTier>chaosLastTierAnnounced){
      chaosLastTierAnnounced=chaosTier;
      mapEventText=chaosTierName();
      mapEventTextTimer=2.4;
      cameraImpactShake=Math.max(cameraImpactShake,.18+chaosTier*.10);
      playImpactSound(chaosTier>=3?"metal":"whoosh",.55+chaosTier*.12);
    }

    if(chaosTier===4 && mayhemTimer<=0){
      mayhemTimer=12;
      gameState.playerProgress.totalMayhem=(gameState.playerProgress.totalMayhem||0)+1;addPlayerXP(28,"MAYHEM");evaluateAchievements();
      chaosMeter=100;
      mapEventText="TOTAL MAYHEM • 2X REWARDS";
      mapEventTextTimer=3.4;
      cameraImpactShake=Math.max(cameraImpactShake,.75);
      if(npc&&!npc.dead){
        npc.anger=100;
        npc.stun=Math.max(npc.stun,.16);
      }
    }
  }

  function cashChaosBank(){
    const reward=Math.floor(chaosRewardBank);
    if(reward>0){
  ...
  function updateChaosDirector(dt){
    ...
    chaosComboTimer=Math.max(0,chaosComboTimer-dt);
    if(chaosComboTimer<=0)chaosCombo=0;

    if(mayhemTimer>0){
      mayhemTimer=Math.max(0,mayhemTimer-dt);
      chaosMeter=100;
      if(npc&&!npc.dead){
        npc.anger=Math.max(npc.anger,88);
        npc.archetype.speed=Math.max(npc.archetype.speed||1.3,1.48);
        npc.archetype.attackMul=Math.max(npc.archetype.attackMul||1,1.12);
      }
      if(mayhemTimer<=0){
        cashChaosBank();
        chaosMeter=48;
        chaosTier=2;
        chaosLastTierAnnounced=2;
        mapEventText="MAYHEM CLEARED";
        mapEventTextTimer=2.2;
      }
      return;
    }

    const decay=(chaosComboTimer>0?.65:2.25)*dt;
    chaosMeter=Math.max(0,chaosMeter-decay);
    const next=chaosMeter>=72?3:chaosMeter>=42?2:chaosMeter>=18?1:0;
    if(next<chaosTier){
      chaosTier=next;
      chaosLastTierAnnounced=Math.min(chaosLastTierAnnounced,next);
    }
  }

  function resetChaosDirector(){
    chaosMeter=0;chaosCombo=0;chaosComboTimer=0;chaosTier=0;mayhemTimer=0;chaosRewardBank=0;chaosLastTierAnnounced=0;
  }

  // Short run objectives create a reason to interact with the whole office,
  // not only the current enemy. They are deliberately simple/readable in VR.
  let runObjective=null;
  const RUN_OBJECTIVES=[
    {id:"smash",label:"PROPERTY DAMAGE",goal:5,reward:55,gem:0},
    {id:"block",label:"NOT TODAY",goal:3,reward:60,gem:0},
    {id:"damage",label:"HR NIGHTMARE",goal:120,reward:70,gem:0},
    {id:"ko",label:"CLEAR THE FLOOR",goal:2,reward:80,gem:1}
  ];

  function chooseRunObjective(){
    const pool=RUN_OBJECTIVES.filter(o=>!(o.id==="ko"&&gameState.mode==="NORMAL"));
    const base=pool[Math.floor(Math.random()*pool.length)];
    runObjective={...base,claimed:false};
  }

  function runObjectiveProgress(){
    if(!runObjective)return 0;
    if(runObjective.id==="smash")return Math.max(0,gameState.destroyed-(runStats.destroyedStart||0));
    if(runObjective.id==="block")return runStats.blocks;
    if(runObjective.id==="damage")return Math.round(runStats.damage);
    if(runObjective.id==="ko")return runStats.kills;
    return 0;
  }

  function updateRunObjective(){
    if(gameMode!=="map"||!runObjective||runObjective.claimed)return;
    const progress=runObjectiveProgress();
    if(progress<runObjective.goal)return;
    runObjective.claimed=true;
    gameState.coins+=runObjective.reward;
    gameState.gems+=runObjective.gem||0;
    gameState.playerProgress.totalObjectives=(gameState.playerProgress.totalObjectives||0)+1;
    addPlayerXP(45,"OBJECTIVE");registerStyle("objective",70);
    registerChaos(18,"objective");
    mapEventText=`OBJECTIVE COMPLETE • +${runObjective.reward} COINS${runObjective.gem?` • +${runObjective.gem} GEM`:""}`;
    mapEventTextTimer=3.3;
    cameraImpactShake=Math.max(cameraImpactShake,.35);
    playImpactSound("metal",.75);
    saveGame();
  }

  function runObjectiveHud(){
    if(!runObjective)return "";
    const p=Math.min(runObjective.goal,runObjectiveProgress());
    return `${runObjective.claimed?"✓ ":""}${runObjective.label} ${p}/${runObjective.goal}`;
  }

  // ------------------------------------------------------------
  // STYLE + OFFICE INCIDENT DIRECTOR — Gauntlet 03/04
  // Rewards variety instead of mindless hit-spam. No locomotion hooks.
  // ------------------------------------------------------------
  let styleScore=0,styleChain=0,styleTimer=0,lastStyleAction="",officeIncidentTimer=18;
  function styleRank(){return styleScore>=850?"S":styleScore>=560?"A":styleScore>=330?"B":styleScore>=160?"C":"D";}
  function registerStyle(action,points){
    if(gameMode!=="map")return;
    const variety=action!==lastStyleAction?1.28:.78;
    styleChain=styleTimer>0?Math.min(12,styleChain+1):1;
    styleTimer=3.4;lastStyleAction=action;
    styleScore+=Math.round(points*variety*(1+Math.min(6,styleChain-1)*.06));
    const pr=gameState.playerProgress;if(pr)pr.bestStyle=Math.max(pr.bestStyle||0,styleScore);
  }
  function updateStyle(dt){if(gameMode!=="map")return;styleTimer=Math.max(0,styleTimer-dt);if(styleTimer<=0)styleChain=0;}
  function incidentLabel(id){return ({printer:"PRINTER MELTDOWN",coffee:"COFFEE PANIC",deadline:"DEADLINE MODE",audit:"SURPRISE AUDIT",meeting:"MEETING FROM HELL"})[id]||"OFFICE INCIDENT";}
  function triggerOfficeIncident(){
    if(gameMode!=="map"||gameState.selectedMap!=="office")return;
    const pool=["printer","coffee","deadline","audit","meeting"];const id=pool[Math.floor(Math.random()*pool.length)];
    mapEventText=incidentLabel(id);mapEventTextTimer=3.4;cameraImpactShake=Math.max(cameraImpactShake,.28);
    if(npc&&!npc.dead){
      if(id==="deadline"){npc.anger=Math.max(npc.anger,86);npc.attackCooldown=Math.min(npc.attackCooldown,.18);}
      if(id==="audit"){npc.stun=Math.max(npc.stun,.55);npc.emotion="scared";speakNpc("scared",false);}
      if(id==="meeting"){npc.behavior={...(npc.behavior||{}),attack:1.18,speed:1.12,hesitate:.72};}
      if(id==="coffee"){npc.throwCooldown=0;npc.anger=Math.max(npc.anger,72);}
    }
    if(id==="printer"){
      const props=officeProps.filter(p=>!p.broken&&["printer","keyboard","mouse","phone"].includes(p.type)).slice(0,3);
      for(const prop of props){prop.loose=true;prop.sleepTimer=0;prop.vel.addInPlace(new BABYLON.Vector3((Math.random()-.5)*2.4,1.2+Math.random(),(Math.random()-.5)*2.4));}
    }
    registerStyle("incident",28);registerChaos(7,"incident");playImpactSound("whoosh",.75);
    if(net.isHost&&net.connected)broadcastMatch({t:"mapEvent",map:"office",label:incidentLabel(id)});
  }

  let survivalLives=0,endlessHeat=0,hardcoreBank=0;
  function modeRewardMultiplier(){return gameState.mode==="HARDCORE"?1.35:gameState.mode==="ENDLESS"?1.18:gameState.mode==="SURVIVAL"?1.10:1;}
  function modeHud(){
    if(gameState.mode==="SURVIVAL")return `SURVIVAL • ${survivalLives} ${survivalLives===1?"LIFE":"LIVES"}`;
    if(gameState.mode==="ENDLESS")return `ENDLESS • HEAT ${endlessHeat}`;
    if(gameState.mode==="HARDCORE")return `HARDCORE • BANK ${Math.floor(hardcoreBank)}`;
    return "";
  }
  function onModeKillReward(){
    if(gameState.mode==="SURVIVAL"){playerHP=Math.min(PLAYER_MAX_HP,playerHP+8);addPlayerXP(8,"SURVIVAL");}
    else if(gameState.mode==="ENDLESS"){endlessHeat=Math.min(50,endlessHeat+1);addPlayerXP(10+Math.floor(endlessHeat*.35),"ENDLESS");}
    else if(gameState.mode==="HARDCORE"){hardcoreBank+=22+Math.min(80,runStats.kills*3);addPlayerXP(16,"HARDCORE");}
  }
  function cashHardcoreBank(){if(hardcoreBank<=0)return;gameState.coins+=Math.floor(hardcoreBank);hardcoreBank=0;saveGame();}
  ...
  function resetRunStats(forcedModifier=null){
    ...
    mapEventTextTimer=0;
    resetChaosDirector();
    survivalLives=gameState.mode==="SURVIVAL"?3:0;endlessHeat=0;hardcoreBank=0;
    styleScore=0;styleChain=0;styleTimer=0;lastStyleAction="";officeIncidentTimer=14+Math.random()*10;
    chooseRunObjective();
    masterResetRun();
  }
  ...
  function applyLocalMapEvent(mapId,label=null){
    ...
    if(mapId==="space"){
      ...
      if(!playerDead&&!playerDowned){playerHP=Math.min(PLAYER_MAX_HP,playerHP+5);updatePlayerHud();}
    }else if(["school","bank","police"].includes(mapId)){
      if(npc&&!npc.dead)npc.stun=Math.max(npc.stun,.35);
    }else if(["arcade","lab","alien"].includes(mapId)){
      abilityCooldown=Math.max(0,abilityCooldown-.35);
    }else if(mapId==="office"){
      const live=officeProps.filter(p=>!p.broken&&!p.heldBy&&p.mass<=3.6);
      if(live.length){const p=live[Math.floor(Math.random()*live.length)];p.loose=true;p.vel.addInPlace(new BABYLON.Vector3((Math.random()-.5)*2.0,.8,(Math.random()-.5)*2.0));}
    }
  ...
  function updateMapSystems(dt){
    ...
    updateStyle(dt);
    if(gameState.selectedMap==="office"){officeIncidentTimer-=dt;if(officeIncidentTimer<=0){triggerOfficeIncident();officeIncidentTimer=18+Math.random()*14;}}
    if(mapEventTimer<=0){
      triggerMapEvent();
      mapEventTimer=(gameState.mode==="HARDCORE"?9:gameState.mode==="ENDLESS"?10:12)+Math.random()*10;
    }
    if(npc&&!npc.dead){
      updateBossPhases();
      ...
      if(gameState.mode==="ENDLESS"&&endlessHeat>0){
        npc.archetype.speed*=1+Math.min(.28,endlessHeat*.008);
        npc.archetype.attackMul*=1+Math.min(.38,endlessHeat*.011);
      }
  ...
  function endRunSummary(bossWin=false){
    ...
    return `${bossWin?"BOSS CLEARED!":"RUN"} • ${gameState.mode} • ${seconds}s • STYLE ${styleRank()} ${styleScore} • KOs ${runStats.kills} • Blocks ${runStats.blocks} • Perfect ${gameState.playerProgress.perfectBlocks||0} • Hardest ${runStats.hardestHit.toFixed(1)} m/s • LV ${gameState.playerProgress.level} • +${c} coins • +${g} gems`;
  }
...
function attachBatToRightHand(){return attachBatToSelectedHand();}
  ...
  function damageNpc(hitPos,swingVel,speed) {
    ...
    const damage=Math.max(1,Math.round(baseDamage*hit.mult*chargedHitMultiplier*twoHandedBonus*masterDamageMultiplier()));
    damageBatDurability(Math.max(.18,(speed-1.4)*.24*(hit.zone==="head"?1.15:1)),'npc');
    ...
    if(finalDamage>=3){addPlayerXP(Math.max(1,Math.round(finalDamage*.16)),"HIT");registerStyle(hit.zone==="head"?"headshot":"hit",hit.zone==="head"?18:9);}
    registerChaos(Math.min(11,1.5+finalDamage*.32+(speed>5.2?2.5:0)),"hit");
    try{onPolishCombatHit(hit,finalDamage,speed,hitPos);}catch(_){ }
  ...
  function updateAdaptiveNpc(){
    ...
    if(!npc||npc.dead||currentMapProgress().level>=10)return;
...
FREE HAND + TRIGGER`;
  ...
  // ------------------------------------------------------------
  // Physical office grab / throw layer. The support hand can pick up nearby
  // non-fixed props with grip. Multiplayer ownership is host-validated and
  // moving/broken props are replicated authoritatively by the host.
  // ------------------------------------------------------------
  const heldOfficeProp={left:null,right:null};
  const grabPrev={left:null,right:null};
  const grabVel={left:new BABYLON.Vector3(),right:new BABYLON.Vector3()};
  function isPropGrabbable(p){
    return !!p&&!p.broken&&!p.heldBy&&p.mass<=3.6&&!['desk','door','cooler','whiteboard','alarm','light'].includes(p.type);
  }
  function nearestGrabbableProp(handPos,maxD=.36){
    let best=null,bd=maxD;
    for(const p of officeProps){
      if(!isPropGrabbable(p))continue;
      const wp=p.root.getAbsolutePosition?.()?.clone?.()||p.root.position.clone();
      const d=BABYLON.Vector3.Distance(handPos,wp);
      if(d<bd){bd=d;best=p;}
    }
    return best;
  }
  function beginOfficeGrab(side,p,handPos){
    if(!p)return false;
    p.heldBy=side;p.loose=false;p.vel.set(0,0,0);p.ang.set(0,0,0);p.sleepTimer=0;
    const wp=p.root.getAbsolutePosition?.()?.clone?.()||p.root.position.clone();
    heldOfficeProp[side]={p,offset:wp.subtract(handPos),rot:p.root.rotation.clone()};
    p.hitMeshes.forEach(m=>removeCollision(m));
    pulse(hands[side],.38,42);playImpactSound('pickup',.72);polishAnnounce(`GRABBED ${String(p.type||"OBJECT").toUpperCase()}`,.65,"pickup",.45);return true;
  }
  function releaseOfficeGrab(side,throwing=true){
    const h=heldOfficeProp[side];if(!h)return;
    const p=h.p;heldOfficeProp[side]=null;p.heldBy=null;p.loose=true;p.sleepTimer=0;
    p.vel.copyFrom(throwing?grabVel[side].scale(.92):BABYLON.Vector3.Zero());
    if(p.vel.length()>7)p.vel.normalize().scaleInPlace(7);
    p.ang.set((Math.random()-.5)*2.1,(Math.random()-.5)*2.6,(Math.random()-.5)*2.1);
    p.hitMeshes.forEach((m,i)=>{if(p.initialCollisions[i])addCollision(m);});
    p.thrownByNpc=false;
    if(throwing&&p.vel.length()>1.3){registerStyle('throw',18);registerChaos(3,'throw');playImpactSound('whoosh',.46);}
  }
  function updateOfficeGrabbing(dt){
    if(!isInXR()||gameMode!=="map"){
      for(const side of ['left','right'])if(heldOfficeProp[side])releaseOfficeGrab(side,false);
      return;
    }
    // Reserve the bat hand for combat; support hand becomes the environmental interaction hand.
    const side=oppositeHand(batHandSide()),h=hands[side];if(!h?.node)return;
    const hp=calibratedHandWorld(h);if(!hp)return;
    if(grabPrev[side]){
      grabVel[side]=hp.subtract(grabPrev[side]).scale(1/Math.max(.008,dt));
      if(grabVel[side].length()>9)grabVel[side].normalize().scaleInPlace(9);
    }
    grabPrev[side]=hp.clone();
    const gripping=btn(h,1);
    if(gripping&&!heldOfficeProp[side]){
      const p=nearestGrabbableProp(hp,.38);
      if(p){
        beginOfficeGrab(side,p,hp);
        if(net.connected&&!net.isHost)sendHost({t:"propGrabReq",k:masterPropKey(p),side});
      }
    }else if(!gripping&&heldOfficeProp[side]){
      const p=heldOfficeProp[side].p;
      if(net.connected&&!net.isHost)sendHost({t:"propRelease",k:masterPropKey(p),v:v3arr(grabVel[side])});
      releaseOfficeGrab(side,true);
    }
    const held=heldOfficeProp[side];
    if(held){
      const target=hp.add(held.offset.scale(.18));
      held.p.root.position.copyFrom(target);
      held.p.root.rotation.x=BABYLON.Scalar.Lerp(held.p.root.rotation.x,held.rot.x,.18);
      held.p.root.rotation.z=BABYLON.Scalar.Lerp(held.p.root.rotation.z,held.rot.z,.18);
      held.p.vel.set(0,0,0);held.p.ang.set(0,0,0);
    }
  }
  function handHoldingOfficeProp(side){return !!heldOfficeProp[side];}

  // ------------------------------------------------------------
  // v0.50 GAUNTLET DIRECTOR — polish pass across combat, bosses,
  // destruction, progression, audio and co-op readability.
  // Movement math intentionally remains isolated from this system.
  // ------------------------------------------------------------
  const POLISH_BOSS_PROFILES={
    office:{a:"BOARDROOM SLAM",b:"HOSTILE TAKEOVER",c:"OVERTIME RAGE",tone:"metal"},
    school:{a:"DETENTION SWEEP",b:"BELL SHOCK",c:"FINAL WARNING",tone:"block"},
    hospital:{a:"CODE RED",b:"DEFIB PULSE",c:"NIGHT SHIFT",tone:"metal"},
    factory:{a:"PRESSURE SURGE",b:"MACHINE RUSH",c:"OVERCLOCK",tone:"metal"},
    pirate:{a:"DECK SLAM",b:"CANNON PANIC",c:"CAPTAIN'S FURY",tone:"wood"},
    volcano:{a:"LAVA STOMP",b:"HEAT WAVE",c:"ERUPTION",tone:"metal"},
    space:{a:"GRAVITY PULSE",b:"AIRLOCK PANIC",c:"ZERO-G RAGE",tone:"whoosh"},
    alien:{a:"PSIONIC PULSE",b:"INVASION WAVE",c:"OVERLORD RAGE",tone:"whoosh"}
  };
  let polishLastMode="",polishBossCooldown=5.5,polishBossTelegraph=0,polishBossQueued=null;
  let polishMomentum=0,polishMomentumTimer=0,polishPerfectFlash=0,polishChainFlash=0;
  let polishLastBossPhase=0,polishIntensity=0,polishLowHpWarn=0,polishPerfTimer=0;
  const polishVfx=[];

  function polishProfile(){
    const base=POLISH_BOSS_PROFILES[gameState.selectedMap];
    if(base)return base;
    const n=mapEventName();return {a:n+" SLAM",b:n+" RUSH",c:"FINAL PHASE",tone:"metal"};
  }
  function careerCoinMultiplier(){
    const lv=gameState.playerProgress?.level||1;
    return 1+(lv>=15?.05:0)+(lv>=35?.05:0)+(lv>=60?.08:0);
  }
  function careerDamageGuard(){
    const lv=gameState.playerProgress?.level||1;
    return lv>=50?.92:lv>=25?.96:1;
  }
  function careerDamageBonus(){
    const lv=gameState.playerProgress?.level||1;
    return lv>=60?1.10:lv>=35?1.07:lv>=15?1.04:1;
  }
  function polishAnnounce(text,time=2.3,sound="metal",strength=.75){
    mapEventText=text;mapEventTextTimer=Math.max(mapEventTextTimer,time);
    if(sound)playImpactSound(sound==="perfect"?"block":sound,strength);
  }
  function spawnPolishRing(pos,kind="impact",scale=1){
    if(!pos||gameState.settings.performanceMode==="PERFORMANCE"&&polishVfx.length>5)return;
    try{
      const mat=mkMat("polishRingMat"+Math.random(),kind==="danger"?"#ff5f45":kind==="perfect"?"#70f0ff":"#ffd166");
      mat.emissiveColor=mat.diffuseColor.scale(kind==="perfect"?1.3:.85);mat.alpha=.78;
      const ring=BABYLON.MeshBuilder.CreateTorus("polishRing",{diameter:.44,thickness:.026,tessellation:24},scene);
      ring.position.copyFrom(pos);ring.rotation.x=Math.PI/2;ring.scaling.setAll(scale);ring.material=mat;
      polishVfx.push({mesh:ring,mat,life:.42,max:.42,grow:kind==="danger"?5.2:3.6});
    }catch(_){ }
  }
  function updatePolishVfx(dt){
    for(let i=polishVfx.length-1;i>=0;i--){
      const v=polishVfx[i];v.life-=dt;
      if(v.life<=0){try{v.mesh.dispose();v.mat.dispose();}catch(_){}polishVfx.splice(i,1);continue;}
      const k=1+dt*v.grow;v.mesh.scaling.scaleInPlace(k);v.mat.alpha=Math.max(0,v.life/v.max*.72);
    }
  }
  function onPolishCombatHit(hit,damage,speed,pos){
    polishMomentum=Math.min(100,polishMomentum+Math.min(20,damage*.65+speed*1.15));
    polishMomentumTimer=2.2;
    if(speed>=6.8||damage>=28){
      spawnPolishRing(pos,"impact",1.1);cameraImpactShake=Math.max(cameraImpactShake,.72);
      polishAnnounce(hit?.zone==="head"?"CRITICAL BONK":"HEAVY IMPACT",.72,"body",1);
      registerStyle("power",22);
    }else if(hit?.zone==="head"&&speed>=3.2){
      spawnPolishRing(pos,"impact",.72);registerStyle("precision",12);
    }
  }
  function onPolishBlock(progress,batSpeed){
    const perfect=progress>.36&&progress<.61&&batSpeed>=2.1;
    if(!perfect)return;
    gameState.playerProgress.perfectBlocks=(gameState.playerProgress.perfectBlocks||0)+1;
    polishPerfectFlash=1.1;polishMomentum=Math.min(100,polishMomentum+28);polishMomentumTimer=3;
    npc.stun=Math.max(npc.stun,1.05);npc.attackCooldown=Math.max(npc.attackCooldown,1.0);
    gameState.coins+=Math.round(3*careerCoinMultiplier());addPlayerXP(6,"PERFECT BLOCK");
    registerChaos(9,"perfectBlock");registerStyle("perfectBlock",70);
    spawnPolishRing(batTip(),"perfect",.85);polishAnnounce("PERFECT BLOCK",1.15,"block",1.12);
    evaluateAchievements();saveGame();
  }
  function onPolishPropDestroyed(prop,center,dir,speed){
    if(gameMode!=="map"||!prop)return;
    let chain=false;
    if(prop.type==="monitor"||prop.type==="printer"){
      if(npc&&!npc.dead&&BABYLON.Vector3.Distance(npc.root.position,center)<2.25){
        npc.stun=Math.max(npc.stun,.55);applyDirectNpcDamage(3);chain=true;
      }
      spawnPolishRing(center,"perfect",.65);
    }else if(prop.type==="cooler"){
      sprinklersActive=true;sprinklerTimer=0;chain=true;polishAnnounce("WATER COOLER DISASTER",1.25,"whoosh",.75);
    }else if(prop.type==="alarm"){
      chain=true;polishAnnounce("SPRINKLERS ACTIVATED",1.6,"metal",.9);
    }else if(prop.type==="light"){
      chain=true;if(npc&&!npc.dead)npc.stun=Math.max(npc.stun,.28);
    }
    if(chain){
      gameState.playerProgress.chainReactions=(gameState.playerProgress.chainReactions||0)+1;
      polishChainFlash=1;registerChaos(10,"chain");registerStyle("chain",45);addPlayerXP(8,"CHAIN");
      spawnPolishRing(center,"danger",.9);saveGame();
    }
  }
  function onPolishBossVictory(){
    const streak=gameState.playerProgress.bossStreak||1;
    const bonus=Math.min(120,20+streak*8);
    gameState.coins+=bonus;addPlayerXP(40+Math.min(80,streak*5),"BOSS");
    polishAnnounce(`BOSS STREAK x${streak} • +${bonus} COINS`,3.2,"metal",1.1);
    if(streak>=3)unlockAchievement("boss_streak_3","MIDDLE MANAGEMENT NIGHTMARE",220,3);
  }
  function resetPolishDirector(){
    polishBossCooldown=4.5;polishBossTelegraph=0;polishBossQueued=null;polishLastBossPhase=0;
    polishMomentum=0;polishMomentumTimer=0;polishPerfectFlash=0;polishChainFlash=0;polishIntensity=0;polishLowHpWarn=0;
  }
  function bossSpecialName(slot){const p=polishProfile();return slot===1?p.a:slot===2?p.b:p.c;}
  function queueBossSpecial(slot){
    if(!npc||npc.dead||npc.typeName==="NPC"||currentMapProgress().level<10)return;
    polishBossQueued=slot;polishBossTelegraph=.95;
    polishAnnounce(`⚠ ${bossSpecialName(slot)}`,1.05,"whoosh",.85);
    npc.attackCooldown=Math.max(npc.attackCooldown,1.15);npc.attackAnim=0;
  }
  function executeBossSpecial(slot){
    if(!npc||npc.dead)return;
    const p=polishProfile(),origin=npc.root.position.add(new BABYLON.Vector3(0,.08,0));
    spawnPolishRing(origin,"danger",slot===3?1.7:1.2);
    cameraImpactShake=Math.max(cameraImpactShake,slot===3?.9:.62);
    if(slot===1){
      npc.stun=Math.max(0,npc.stun-.2);npc.anger=Math.min(100,npc.anger+14);
      if(!playerDead&&!playerDowned&&BABYLON.Vector3.Distance(playerWorldPos(),npc.root.position)<3.1)hurtPlayer(Math.round(8*careerDamageGuard()),npc.root.position);
      playImpactSound(p.tone||"metal",1.05);
    }else if(slot===2){
      npc.throwCooldown=0;npc.anger=100;npc.attackCooldown=.15;
      triggerMapEvent();playImpactSound(p.tone||"metal",1);
    }else{
      npc.anger=100;npc.stun=0;npc.attackCooldown=.05;
      npc.archetype.speed=Math.max(npc.archetype.speed||1.3,(npc.baseSpeed||1.3)*1.28);
      npc.archetype.attackMul=Math.max(npc.archetype.attackMul||1,(npc.baseAttackMul||1)*1.28);
      if(npc.bossArmorMax>0&&npc.bossArmor<=0)npc.bossArmor=Math.round(npc.bossArmorMax*.18);
      playImpactSound(p.tone||"metal",1.15);
    }
  }
  function updateBossDirectorPolish(dt){
    if(gameMode!=="map"||!npc||npc.dead||currentMapProgress().level<10)return;
    const hpPct=npc.hpValue/Math.max(1,npc.maxHp);
    const phase=hpPct>.66?1:hpPct>.33?2:3;
    if(phase!==polishLastBossPhase){
      polishLastBossPhase=phase;polishBossCooldown=2.6;
      polishAnnounce(`BOSS PHASE ${phase}`,1.7,"metal",.9);
      spawnPolishRing(npc.root.position,"danger",1.1+phase*.2);
    }
    if(polishBossTelegraph>0){
      polishBossTelegraph-=dt;if(polishBossTelegraph<=0&&polishBossQueued){const q=polishBossQueued;polishBossQueued=null;executeBossSpecial(q);polishBossCooldown=phase===3?4.2:5.4;}
      return;
    }
    polishBossCooldown-=dt;
    if(polishBossCooldown<=0&&npc.stun<=0&&!npc.recovering)queueBossSpecial(phase);
  }
  function updatePolishDirector(dt){
    updatePolishVfx(dt);
    polishPerfectFlash=Math.max(0,polishPerfectFlash-dt);polishChainFlash=Math.max(0,polishChainFlash-dt);
    if(gameMode!==polishLastMode){polishLastMode=gameMode;resetPolishDirector();}
    if(gameMode!=="map")return;
    polishMomentumTimer=Math.max(0,polishMomentumTimer-dt);
    if(polishMomentumTimer<=0)polishMomentum=Math.max(0,polishMomentum-dt*13);
    const hpPressure=1-BABYLON.Scalar.Clamp(playerHP/Math.max(1,PLAYER_MAX_HP),0,1);
    const bossPressure=currentMapProgress().level>=10?1:0;
    polishIntensity=BABYLON.Scalar.Clamp((chaosMeter/100)*.52+(polishMomentum/100)*.28+hpPressure*.12+bossPressure*.25,0,1);
    if(currentMapProgress().level>=10)setMusicMode("boss");
    else if(polishIntensity>.56||mayhemTimer>0)setMusicMode("mayhem");
    else setMusicMode("normal");
    if(playerHP<=PLAYER_MAX_HP*.25&&!playerDead&&!playerDowned){
      polishLowHpWarn-=dt;if(polishLowHpWarn<=0){polishLowHpWarn=5.5;polishAnnounce("LOW HEALTH",1.1,"body",.55);}
    }
    updateBossDirectorPolish(dt);
    polishPerfTimer+=dt;
    if(polishPerfTimer>2){
      polishPerfTimer=0;
      if(polishVfx.length>12){for(const v of polishVfx.splice(0,polishVfx.length-8)){try{v.mesh.dispose();v.mat.dispose();}catch(_){}}}
    }
  }


  // ------------------------------------------------------------
  // v0.60 MASTER GAUNTLET — systemic depth pass.
  // IMPORTANT: this layer intentionally does not modify hand locomotion math.
  // It deepens AI, boss arenas, improvised combat, run progression and feedback.
  // ------------------------------------------------------------
  let masterRunSeconds=0,masterKillMilestone=0,masterBossHazardCooldown=4.5;
  let masterComboTimer=0,masterComboCount=0,masterPromotionText="";
  const masterRunBuffs={power:0,toughness:0,officeDamage:0,focus:0,recovery:0};
  const masterHazards=[];

  function masterResetRun(){
    masterRunSeconds=0;masterKillMilestone=0;masterBossHazardCooldown=4.5;
    masterComboTimer=0;masterComboCount=0;masterPromotionText="";
    for(const k of Object.keys(masterRunBuffs))masterRunBuffs[k]=0;
    while(masterHazards.length){const h=masterHazards.pop();try{h.mesh?.dispose();h.mat?.dispose();}catch(_){}}
  }

  function masterPromotionLabel(){
    const b=masterRunBuffs;
    const bits=[];
    if(b.power)bits.push(`POWER ${b.power}`);
    if(b.toughness)bits.push(`TOUGH ${b.toughness}`);
    if(b.officeDamage)bits.push(`CHAOS ${b.officeDamage}`);
    if(b.focus)bits.push(`FOCUS ${b.focus}`);
    if(b.recovery)bits.push(`RECOVERY ${b.recovery}`);
    return bits.length?`PROMOTIONS • ${bits.join(" • ")}`:"";
  }

  function masterAwardPromotion(){
    const choices=["power","toughness","officeDamage","focus","recovery"];
    // Bias toward underused upgrades so one run develops rather than snowballs one stat.
    choices.sort((a,b)=>masterRunBuffs[a]-masterRunBuffs[b]+(Math.random()-.5)*.7);
    const pick=choices[0];masterRunBuffs[pick]++;
    const names={power:"POWER PLAY",toughness:"THICK SKIN",officeDamage:"OFFICE MENACE",focus:"LOCKED IN",recovery:"SECOND WIND"};
    masterPromotionText=`PROMOTION • ${names[pick]} LV ${masterRunBuffs[pick]}`;
    mapEventText=masterPromotionText;mapEventTextTimer=3.0;
    polishAnnounce(masterPromotionText,1.25,"pickup",.85);playImpactSound("pickup",.9);
    addPlayerXP(20,"PROMOTION");registerStyle("promotion",50);registerChaos(5,"promotion");
    saveGame();
  }

  function masterDamageMultiplier(){return 1+masterRunBuffs.power*.075;}
  function masterOfficeMultiplier(){return 1+masterRunBuffs.officeDamage*.14;}
  function masterIncomingMultiplier(){return Math.max(.62,1-masterRunBuffs.toughness*.075);}
  function masterRecoveryOnKo(){
    if(masterRunBuffs.recovery<=0)return;
    const heal=2+masterRunBuffs.recovery*2;
    playerHP=Math.min(PLAYER_MAX_HP,playerHP+heal);updatePlayerHud();
  }

  function masterRegisterKo(){
    masterComboTimer=4.0;masterComboCount=Math.min(99,masterComboCount+1);
    masterRecoveryOnKo();
    const milestone=Math.floor((runStats?.kills||0)/3);
    if(milestone>masterKillMilestone){masterKillMilestone=milestone;masterAwardPromotion();}
  }

  // Stronger material identity for environmental combat.
  function masterPropCombatProfile(p){
    const type=String(p?.type||"");
    if(["printer","monitor","stapler"].includes(type))return {damage:1.28,stun:.18,sound:"metal",breakStress:1.15};
    if(["chair","plant","bin"].includes(type))return {damage:1.12,stun:.12,sound:type==="chair"?"plastic":"body",breakStress:1.0};
    if(["mug","pencup"].includes(type))return {damage:1.20,stun:.08,sound:"ceramic",breakStress:.62};
    if(["keyboard","mouse","phone"].includes(type))return {damage:.92,stun:.06,sound:"plastic",breakStress:.72};
    return {damage:1,stun:.08,sound:"body",breakStress:1};
  }

  // NPC environmental melee: if an enemy has already picked something up and
  // the player closes distance, it can be used as a visible improvised weapon
  // instead of always being thrown.
  let masterNpcPropSwing=0,masterNpcPropHit=false,masterNpcPropCooldown=0;
  function masterUpdateNpcPropCombat(dt){
    masterNpcPropCooldown=Math.max(0,masterNpcPropCooldown-dt);
    if(!npcHeldOfficeProp||!npc||npc.dead||npc.recovering)return;
    const p=npcHeldOfficeProp.p;if(!p||p.broken)return;
    const target=npcCombatTarget?.();if(!target)return;
    const d=BABYLON.Vector3.Distance(target.pos,npc.root.position);
    if(masterNpcPropSwing<=0 && d<1.65 && masterNpcPropCooldown<=0 && npc.stun<=0){
      masterNpcPropSwing=.46;masterNpcPropHit=false;masterNpcPropCooldown=1.0+Math.random()*.65;
      speakNpc("attack",false);playImpactSound("whoosh",.68);
    }
    if(masterNpcPropSwing>0){
      masterNpcPropSwing=Math.max(0,masterNpcPropSwing-dt);
      const t=1-masterNpcPropSwing/.46;
      const hand=npc.rightHand?.getAbsolutePosition?.()||npc.root.position.add(new BABYLON.Vector3(.32,1.15,.1));
      const forward=target.pos.subtract(npc.root.position);forward.y=0;if(forward.lengthSquared()<.001)forward.set(0,0,1);forward.normalize();
      const side=new BABYLON.Vector3(forward.z,0,-forward.x);
      const arc=Math.sin(t*Math.PI);
      p.root.position.copyFrom(hand.add(forward.scale(.2+arc*.55)).add(side.scale((.5-t)*.45)).add(new BABYLON.Vector3(0,.05+arc*.18,0)));
      p.root.rotation.x+=dt*(5+arc*8);p.root.rotation.z+=dt*5;
      if(!masterNpcPropHit && t>.34 && t<.82){
        const pp=p.root.getAbsolutePosition?.()?.clone?.()||p.root.position.clone();
        if(BABYLON.Vector3.Distance(pp,target.chest||target.pos)<.72){
          masterNpcPropHit=true;
          const prof=masterPropCombatProfile(p);const dmg=Math.max(3,Math.round((5+(p.mass||1)*1.8)*prof.damage));
          damageCombatTarget(target,dmg,npc.root.position);playImpactSound(prof.sound,.95);spawnPolishRing(pp,"impact",.78);
          if(target.local)polishAnnounce(`${String(p.type||"OFFICE").toUpperCase()} SMACK • -${dmg}`,1.0,prof.sound,.9);
        }
      }
      if(masterNpcPropSwing<=0 && Math.random()<.28)dropNpcHeldObject(null);
    }
  }

  // Lightweight squad roles: one pressures center while the other tries to
  // flank. It only influences the simple extra-NPC roots; no expensive pathfinder.
  function masterUpdateSquadTactics(dt){
    if(gameMode!=="map"||!isInXR()||playerDead)return;
    const pp=playerWorldPos();
    const live=extraNpcs.filter(e=>e.root?.isEnabled?.()&&!e.dead);
    for(let i=0;i<live.length;i++){
      const e=live[i];e.masterRole=e.masterRole||(i%2?"FLANKER":"PRESSURE");
      if(e.heldProp)continue;
      const to=pp.subtract(e.root.position);to.y=0;const d=to.length();if(d<.01)continue;to.normalize();
      const side=new BABYLON.Vector3(to.z,0,-to.x);
      const desired=e.masterRole==="FLANKER"?to.scale(.42).add(side.scale(i%2?.62:-.62)):to;
      if(d>2.0 && d<8.5){
        desired.normalize();e.root.position.addInPlace(desired.scale(dt*(e.masterRole==="FLANKER"?1.05:.72)));
        e.root.rotation.y=Math.atan2(to.x,to.z);
      }
    }
  }

  function masterSpawnHazard(pos,kind="pulse",radius=1.5,delay=.9){
    const mat=mkMat(`masterHazardMat${performance.now()}`,kind==="boss"?"#ff4b64":"#ffba52");
    mat.alpha=.22;mat.emissiveColor=mat.diffuseColor.scale(.7);
    const mesh=BABYLON.MeshBuilder.CreateTorus(`masterHazard${performance.now()}`,{diameter:radius*2,thickness:.055,tessellation:32},scene);
    mesh.position.copyFrom(pos);mesh.position.y=.035;mesh.rotation.x=Math.PI/2;mesh.material=mat;
    masterHazards.push({mesh,mat,life:delay+.45,delay,radius,hit:false});
  }

  function masterUpdateHazards(dt){
    for(let i=masterHazards.length-1;i>=0;i--){
      const h=masterHazards[i];h.life-=dt;h.delay-=dt;
      const pulse=1+Math.sin(performance.now()*.012)*.04;h.mesh.scaling.set(pulse,pulse,pulse);
      if(h.delay<=0&&!h.hit){
        h.hit=true;h.mat.alpha=.55;playImpactSound("metal",1.0);playSpatialTick(h.mesh.position,.9);cameraImpactShake=Math.max(cameraImpactShake,.5);
        const pp=playerWorldPos();const flat=new BABYLON.Vector3(pp.x,h.mesh.position.y,pp.z);
        if(BABYLON.Vector3.Distance(flat,h.mesh.position)<h.radius && !playerDead&&!playerDowned){
          hurtPlayer(Math.round((9+currentMapProgress().level*.7)*masterIncomingMultiplier()),h.mesh.position);
        }
      }
      if(h.life<=0){try{h.mesh.dispose();h.mat.dispose();}catch(_){}masterHazards.splice(i,1);}
    }
  }

  function masterBossArenaDirector(dt){
    if(gameMode!=="map"||!npc||npc.dead||currentMapProgress().level<10)return;
    masterBossHazardCooldown-=dt;
    if(masterBossHazardCooldown>0)return;
    masterBossHazardCooldown=Math.max(3.4,7.2-(npc.bossPhase||1)*.75)+Math.random()*2.0;
    const pp=playerWorldPos();
    const center=pp.add(new BABYLON.Vector3((Math.random()-.5)*1.7,0,(Math.random()-.5)*1.7));center.y=.02;
    const radius=1.1+(npc.bossPhase||1)*.18;
    masterSpawnHazard(center,"boss",radius,.85);
    mapEventText=`BOSS TELEGRAPH • MOVE!`;mapEventTextTimer=1.0;
    playImpactSound("whoosh",.8);
  }

  // Better spatial-ish feedback using StereoPanner. Kept procedural so the
  // build remains self-contained and copyright-clean.
  function playSpatialTick(pos,strength=.5){
    const ac=audioCtx();if(!ac||!pos)return;
    try{
      const cam=xrCamera?.globalPosition||xrCamera?.position||scene.activeCamera?.globalPosition||scene.activeCamera?.position;if(!cam)return;
      const dx=pos.x-cam.x,dz=pos.z-cam.z;const pan=BABYLON.Scalar.Clamp(dx/Math.max(1,Math.abs(dx)+Math.abs(dz)),-1,1);
      const o=ac.createOscillator(),g=ac.createGain(),p=ac.createStereoPanner?ac.createStereoPanner():null,now=ac.currentTime;
      o.type="triangle";o.frequency.setValueAtTime(190+strength*130,now);o.frequency.exponentialRampToValueAtTime(90,now+.08);
      g.gain.setValueAtTime(.025*strength*(gameState.settings?.sfxVolume??.75),now);g.gain.exponentialRampToValueAtTime(.001,now+.09);
      o.connect(g);if(p){p.pan.value=pan;g.connect(p);p.connect(ac.destination);}else g.connect(ac.destination);o.start(now);o.stop(now+.1);
    }catch(_){}
  }


  const masterNetPropOwners=new Map();
  function masterHostTryGrabProp(pid,d){
    if(!net.isHost||gameMode!=="map"||!net.matchMembers?.has(pid))return false;
    const entry=net.players.get(pid);const side=d.side==="left"?"left":"right";
    const hand=arrv(entry?.pose?.[side]);if(!hand||!entry?.lastPoseRecv||performance.now()-entry.lastPoseRecv>800)return false;
    const p=officeProps.find(q=>masterPropKey(q)===d.k);if(!p||!isPropGrabbable(p))return false;
    const wp=p.root.getAbsolutePosition?.()?.clone?.()||p.root.position.clone();if(BABYLON.Vector3.Distance(hand,wp)>.62)return false;
    p.heldBy=`net:${pid}`;p.loose=false;p.vel.set(0,0,0);p.ang.set(0,0,0);p.hitMeshes.forEach(m=>removeCollision(m));
    masterNetPropOwners.set(d.k,{pid,side});sendConn(net.connections.get(pid),{t:"propGrabOk",k:d.k});return true;
  }
  function masterHostReleaseProp(pid,d){
    const own=masterNetPropOwners.get(d.k);if(!own||own.pid!==pid)return false;
    const p=officeProps.find(q=>masterPropKey(q)===d.k);masterNetPropOwners.delete(d.k);if(!p)return false;
    p.heldBy=null;p.loose=true;p.sleepTimer=0;p.hitMeshes.forEach((m,i)=>{if(p.initialCollisions[i])addCollision(m);});
    const v=arrv(d.v)||new BABYLON.Vector3();if(v.length()>7)v.normalize().scaleInPlace(7);p.vel.copyFrom(v.scale(.92));p.ang.set((Math.random()-.5)*2.1,(Math.random()-.5)*2.6,(Math.random()-.5)*2.1);p.thrownByNpc=false;
    return true;
  }
  function masterUpdateNetworkHeldProps(){
    if(!net.isHost||!masterNetPropOwners.size)return;
    for(const [k,own] of [...masterNetPropOwners]){
      const entry=net.players.get(own.pid);const hand=arrv(entry?.pose?.[own.side]);const p=officeProps.find(q=>masterPropKey(q)===k);
      if(!p||!hand||!entry?.lastPoseRecv||performance.now()-entry.lastPoseRecv>1200){
        if(p){p.heldBy=null;p.loose=true;p.vel.set(0,.2,0);p.hitMeshes.forEach((m,i)=>{if(p.initialCollisions[i])addCollision(m);});}
        masterNetPropOwners.delete(k);continue;
      }
      p.root.position.copyFrom(hand);p.vel.set(0,0,0);p.ang.set(0,0,0);p.loose=false;
    }
  }
  let masterPropNetTimer=0;
  function masterPropKey(p){return p?.root?.name||String(officeProps.indexOf(p));}
  function masterBroadcastPropState(dt){
    if(!net.isHost||gameMode!=="map"||!net.matchMembers?.size)return;
    masterPropNetTimer-=dt;if(masterPropNetTimer>0)return;masterPropNetTimer=.12;
    const states=[];
    for(const p of officeProps){
      if(!p||(!p.loose&&!p.broken&&!p.heldBy))continue;
      const rp=p.root.position,rr=p.root.rotation;
      states.push({k:masterPropKey(p),p:[+rp.x.toFixed(3),+rp.y.toFixed(3),+rp.z.toFixed(3)],r:[+rr.x.toFixed(3),+rr.y.toFixed(3),+rr.z.toFixed(3)],v:[+p.vel.x.toFixed(2),+p.vel.y.toFixed(2),+p.vel.z.toFixed(2)],l:!!p.loose,b:!!p.broken,h:p.heldBy?1:0});
      if(states.length>=18)break;
    }
    if(states.length)broadcastMatch({t:"props",s:states});
  }
  function masterApplyPropState(d){
    if(!Array.isArray(d?.s))return;
    const byKey=new Map(officeProps.map(p=>[masterPropKey(p),p]));
    for(const st of d.s){
      const p=byKey.get(st.k);if(!p||!Array.isArray(st.p)||!Array.isArray(st.r))continue;
      const locallyHeld=(heldOfficeProp.left?.p===p||heldOfficeProp.right?.p===p);
      if(!locallyHeld){
        p.root.position.set(Number(st.p[0])||0,Number(st.p[1])||0,Number(st.p[2])||0);
        p.root.rotation.set(Number(st.r[0])||0,Number(st.r[1])||0,Number(st.r[2])||0);
      }
      if(Array.isArray(st.v))p.vel.set(Number(st.v[0])||0,Number(st.v[1])||0,Number(st.v[2])||0);
      p.loose=!!st.l;p.heldBy=st.h?"host":null;
      if(st.b&&!p.broken){p.broken=true;p.root.setEnabled(false);}
      else if(!st.b&&p.broken){p.broken=false;p.root.setEnabled(true);}
    }
  }
  ...
  // ------------------------------------------------------------
  // v0.61 FINAL GAUNTLET — session pacing, readability and long-run stability.
  // No locomotion math is touched here.
  // ------------------------------------------------------------
  let finalPacePressure=0,finalBreather=0,finalCleanupTimer=3.0,finalDangerCue=0;
  let finalLastIncident="",finalIncidentRepeatGuard=0;
  function finalPlayerPressure(){
    const hp=Math.max(0,Math.min(1,playerHP/Math.max(1,PLAYER_MAX_HP)));
    const enemies=(npc&&!npc.dead?1:0)+extraNpcs.filter(e=>!e.dead&&e.root?.isEnabled?.()).length;
    const boss=(npc&&!npc.dead&&currentMapProgress().level>=10)?1:0;
    return BABYLON.Scalar.Clamp((1-hp)*.62+Math.min(.28,enemies*.07)+boss*.14+(playerDowned?.35:0),0,1);
  }
  function finalUpdatePacing(dt){
    if(gameMode!=="map")return;
    const target=finalPlayerPressure();
    finalPacePressure=BABYLON.Scalar.Lerp(finalPacePressure,target,Math.min(1,dt*2.4));
    finalBreather=Math.max(0,finalBreather-dt);finalDangerCue=Math.max(0,finalDangerCue-dt);
    // Fairness director: when a player is nearly overwhelmed, create a short readable recovery window
    // instead of silently nerfing damage or handing out free health.
    if(finalPacePressure>.82&&finalBreather<=0&&!playerDead&&!playerDowned){
      finalBreather=4.2;
      if(npc&&!npc.dead){npc.attackCooldown=Math.max(npc.attackCooldown,1.05);npc.throwCooldown=Math.max(npc.throwCooldown,.85);}
      for(const e of extraNpcs){if(!e.dead)e.cooldown=Math.max(e.cooldown,.75+Math.random()*.4);}
      polishAnnounce("CATCH YOUR BREATH",1.1,"whoosh",.4);
    }
    if(finalPacePressure>.68&&finalDangerCue<=0&&playerHP<=PLAYER_MAX_HP*.32){
      finalDangerCue=5.0;
      playSpatialTick(playerWorldPos(),.38);
    }
    finalIncidentRepeatGuard=Math.max(0,finalIncidentRepeatGuard-dt);
  }
  function finalCleanupLongSession(dt){
    finalCleanupTimer-=dt;if(finalCleanupTimer>0)return;finalCleanupTimer=3.0;
    // Defensive caps for long Quest sessions. Existing systems still own normal disposal.
    while(polishVfx.length>Math.max(8,PERF.maxFx||18)){const v=polishVfx.shift();try{v?.mesh?.dispose();v?.mat?.dispose();}catch(_){}}
    while(masterHazards.length>6){const h=masterHazards.shift();try{h?.mesh?.dispose();h?.mat?.dispose();}catch(_){}}
    for(const p of officeProps){
      if(!p||p.broken||p.heldBy)continue;
      if(p.vel?.length?.()>14)p.vel.normalize().scaleInPlace(14);
      if(p.ang?.length?.()>18)p.ang.normalize().scaleInPlace(18);
    }
  }
  function finalReadableThreats(){
    if(gameMode!=="map"||!npc||npc.dead)return;
    // Keep boss/special attacks readable even during maximum Mayhem.
    if(polishBossTelegraph>0||masterHazards.some(h=>h.delay>0)){
      npc.attackCooldown=Math.max(npc.attackCooldown,.18);
    }
  }
  function finalGauntletUpdate(dt){
    finalUpdatePacing(dt);finalCleanupLongSession(dt);finalReadableThreats();
  }

  function masterUpdate(dt){
    if(gameMode!=="map"){masterRunSeconds=0;return;}
    masterRunSeconds+=dt;masterComboTimer=Math.max(0,masterComboTimer-dt);if(masterComboTimer<=0)masterComboCount=0;
    masterUpdateNpcPropCombat(dt);masterUpdateSquadTactics(dt);masterBossArenaDirector(dt);masterUpdateHazards(dt);masterUpdateNetworkHeldProps();masterBroadcastPropState(dt);finalGauntletUpdate(dt);
  }
  ...
  scene.onBeforeRenderObservable.add(()=>{
    ...
    if (playerDead) {
      ...
      if (deathTimer<=0) {
        if(gameState.mode==="HARDCORE") {
          hardcoreBank=0;gameState.playerProgress.bossStreak=0;saveGame();
          respawnPlayer();goLobby();lastLobbyMessage=`HARDCORE RUN ENDED • STYLE ${styleRank()} ${styleScore}`;refreshLobby();
        } else if(gameState.mode==="SURVIVAL") {
          survivalLives=Math.max(0,survivalLives-1);
          if(survivalLives<=0){
            gameState.playerProgress.bossStreak=0;saveGame();
            respawnPlayer();goLobby();lastLobbyMessage=`SURVIVAL OVER • ${runStats.kills} KOs • STYLE ${styleRank()} ${styleScore}`;refreshLobby();
          }else{
            respawnPlayer();mapEventText=`${survivalLives} ${survivalLives===1?"LIFE":"LIVES"} LEFT`;mapEventTextTimer=2.8;
          }
        } else {
          respawnPlayer();
        }
    ...
    if (npc && !(net.connected&&!net.isHost)) {
      ...
      if(npc.reactionCooldown<=0 && npc.hpValue>0){
        npc.voiceCategory=null;
        npc.voiceClipId=null;
      }
      ...
      if (!npc.dead) {
        ...
        if (!npc.recovering && isInXR() && !playerDead && gameMode==="map") {
          ...
          if (
            ...
            let speed=(npc.archetype?.speed || 1.30)*(npc.behavior?.speed||1);
            if (npc.emotion==="angry") speed=Math.max(speed,1.72*(npc.behavior?.speed||1));
            if (npc.emotion==="scared") speed=1.18*(npc.behavior?.speed||1);
          ...
          if(
            npc.anger>=68 &&
            npcIsFullyUpright() &&
            d>Math.max(1.8,(npc.behavior?.preferred||2.3)-.25) && d<5.6 &&
            npc.throwCooldown<=0 &&
            npc.stun<=0 &&
            npc.attackAnim<=0
          ){
            const objAction=npcThrowNearbyObject(playerPos);
            if(objAction==='thrown'){
              npc.throwCooldown=(3.0+Math.random()*1.7)/((npc.archetype?.throwRate||1)*(npc.behavior?.throw||1));npc.attackCooldown=.55;
            }else if(objAction==='picked'||objAction==='holding'){
              npc.throwCooldown=.34;npc.attackCooldown=Math.max(npc.attackCooldown,.25);
            }else npc.throwCooldown=1.1;
          }

          // Start a real weapon swing, but distance alone does NOT deal damage.
          if (
            d<=Math.max(2.05,(npc.behavior?.preferred||1.7)+.72) &&
            npcIsFullyUpright() &&
            npc.stun<=0 &&
            npc.attackAnim<=0 &&
            ...
            npc.attackCooldown=((npc.archetype?.attackRate ?? .38)*(npc.behavior?.hesitate||1))+armPenalty*.42;
          ...
          if (npc.attackAnim>0) {
            ...
            if (!npc.attackBlocked && liveCombatTarget.local && batRoot.isEnabled() && !batHolstered) {
              ...
              if(blockDist<.205 && progress>.16 && progress<.95){
                ...
                damageBatDurability(Math.max(.7,1.1+blockBatSpeed*.22),'block');
                recordBlock();
                try{onPolishBlock(progress,blockBatSpeed);}catch(_){ }
            ...
            if (!npc.attackBlocked && !npc.attackHasHit && progress>.34) {
              ...
              if(hit){
                ...
                const dm=npcDifficultyMul();const attackDamage=Math.max(1,Math.round(npc.weaponCfg.damage*(npc.archetype?.attackMul||1)*(npc.behavior?.attack||1)*armWeak*1.08*dm.damage));
    ...
    if (xrCamera && xr?.baseExperience?.state===BABYLON.WebXRState.IN_XR) {
      if (!playerDead) {
        for (const side of ["left","right"]) {
          ...
          if(!handHoldingOfficeProp(side))updateHandLocomotion(h,wp,tp,dt);
          else {h.contact=false;h.anchor=null;h.normal=null;h.mesh.position.copyFrom(tp);}
  ...
  let npcHeldOfficeProp=null;
  function npcPickupNearbyObject(){
    if(!npc||npc.dead||npcHeldOfficeProp)return false;
    const candidates=officeProps.filter(p=>
      isPropGrabbable(p) &&
      ["mug","keyboard","mouse","pencup","stapler","bin","plant","chair","printer"].includes(p.type) &&
      BABYLON.Vector3.Distance(p.root.getAbsolutePosition?.()||p.root.position,npc.root.position)<2.35
    );
    if(!candidates.length)return false;
    const p=candidates[Math.floor(Math.random()*candidates.length)];
    p.heldBy="npc";p.loose=false;p.vel.set(0,0,0);p.ang.set(0,0,0);p.thrownByNpc=false;
    p.hitMeshes.forEach(m=>removeCollision(m));
    npcHeldOfficeProp={p,holdTime:0};
    playImpactSound('pickup',.7);speakNpc(Math.random()<.5?'angry':'furious',false);
    return true;
  }
  function dropNpcHeldObject(throwAt=null){
    if(!npcHeldOfficeProp)return false;
    const p=npcHeldOfficeProp.p;npcHeldOfficeProp=null;p.heldBy=null;p.loose=true;p.sleepTimer=0;
    p.hitMeshes.forEach((m,i)=>{if(p.initialCollisions[i])addCollision(m);});
    if(throwAt){
      const pos=p.root.getAbsolutePosition?.()?.clone?.()||p.root.position.clone();let dir=throwAt.subtract(pos);
      if(dir.lengthSquared()<.001)dir.set(0,.05,1);dir.normalize();
      const power=4.7+Math.min(1.6,(npc.anger||0)*.012)+(npc.behavior?.throw||1)*.35;
      p.vel.copyFrom(dir.scale(power).add(new BABYLON.Vector3(0,1.15,0)));p.thrownByNpc=true;p.playerHitCooldown=.12;
      p.ang.set((Math.random()-.5)*7,(Math.random()-.5)*8,(Math.random()-.5)*7);playImpactSound('whoosh',.78);speakNpc('attack',false);
    }else{p.vel.set((Math.random()-.5)*.8,.4,(Math.random()-.5)*.8);p.thrownByNpc=false;}
    return true;
  }
  function updateNpcHeldObject(dt){
    if(!npcHeldOfficeProp)return;
    if(!npc||npc.dead||npc.recovering||npc.stun>.35){dropNpcHeldObject(null);return;}
    const p=npcHeldOfficeProp.p;if(!p||p.broken){npcHeldOfficeProp=null;return;}
    npcHeldOfficeProp.holdTime+=dt;
    const hand=npc.rightHand?.getAbsolutePosition?.()||npc.root.position.add(new BABYLON.Vector3(.34,1.15,.12));
    const fwd=new BABYLON.Vector3(Math.sin(npc.root.rotation.y),0,Math.cos(npc.root.rotation.y));
    p.root.position.copyFrom(hand.add(fwd.scale(.18)).add(new BABYLON.Vector3(0,.06,0)));
    p.root.rotation.x+=dt*1.8;p.root.rotation.y+=dt*2.2;p.vel.set(0,0,0);p.ang.set(0,0,0);
  }
  function npcThrowNearbyObject(playerPos){
    if(!npc||npc.dead)return false;
    if(!npcHeldOfficeProp){
      if(npcPickupNearbyObject())return 'picked';
      return false;
    }
    if(npcHeldOfficeProp.holdTime<.28)return 'holding';
    dropNpcHeldObject(playerPos);return 'thrown';
  }
  ...
  function makeExtraNpc(index){
    ...
    const e={
      ...
      respawn:0,
      hitFlash:0,
      objectCooldown:1.2+Math.random()*1.5,
      heldProp:null,heldPropTime:0
    };
  ...
  function configureExtraSquad(){
    ...
    extraNpcs.forEach((e,i)=>{
      ...
      if(on){
        if(e.heldProp){const hp=e.heldProp;e.heldProp=null;hp.heldBy=null;hp.loose=true;hp.thrownByNpc=false;}
        e.dead=false;e.hpValue=e.maxHp;e.velocity.set(0,0,0);e.objectCooldown=1+Math.random()*1.5;e.heldPropTime=0;
  ...
  function damageExtraNpc(hit,swingVel,speed){
    ...
    if(e.hpValue<=0){
      e.dead=true;
      e.hpValue=0;
      runStats.kills++;gameState.kills++;onModeKillReward();masterRegisterKo();addPlayerXP(22,"SQUAD KO");gameState.coins+=Math.round(10*chaosRewardMultiplier());
      registerChaos(16,"squadKo");registerStyle("squadKo",42);progressDaily("kill");addBatXP(12);evaluateAchievements();saveGame();
  ...
  function updateExtraNpcs(dt){
    ...
    for(let i=0;i<extraNpcs.length;i++){
      ...
      if(e.dead){
        if(e.heldProp){const p=e.heldProp;e.heldProp=null;p.heldBy=null;p.loose=true;p.thrownByNpc=false;p.hitMeshes.forEach((m,j)=>{if(p.initialCollisions[j])addCollision(m);});}
        e.respawn-=dt;
        if(e.respawn<=0){
          e.dead=false;
          e.hpValue=e.maxHp;
      ...
      e.objectCooldown=Math.max(0,(e.objectCooldown||0)-dt);
      if(e.heldProp){
        e.heldPropTime=(e.heldPropTime||0)+dt;const p=e.heldProp;
        const side=new BABYLON.Vector3(Math.cos(e.root.rotation.y),0,-Math.sin(e.root.rotation.y));
        p.root.position.copyFrom(e.root.position.add(new BABYLON.Vector3(0,1.25,0)).add(side.scale(.34)).add(to.scale(.10)));
        p.root.rotation.y+=dt*2.1;p.root.rotation.x+=dt*1.3;p.vel.set(0,0,0);p.ang.set(0,0,0);
        if(e.heldPropTime>.38 && d>1.75 && d<6.2){
          e.heldProp=null;p.heldBy=null;p.loose=true;p.sleepTimer=0;p.thrownByNpc=true;p.playerHitCooldown=.12;
          p.hitMeshes.forEach((m,j)=>{if(p.initialCollisions[j])addCollision(m);});
          const pos=p.root.getAbsolutePosition?.()?.clone?.()||p.root.position.clone();let td=pp.subtract(pos);if(td.lengthSquared()<.001)td.set(0,.1,1);td.normalize();
          p.vel.copyFrom(td.scale(4.5).add(new BABYLON.Vector3(0,1.0,0)));p.ang.set((Math.random()-.5)*7,(Math.random()-.5)*7,(Math.random()-.5)*7);
          playImpactSound('whoosh',.62);e.objectCooldown=3.0+Math.random()*2.2;e.heldPropTime=0;
        }
      }else if(e.objectCooldown<=0 && d>2.0 && d<5.7){
        const cand=officeProps.filter(p=>isPropGrabbable(p)&&["mug","keyboard","mouse","pencup","stapler","bin","plant"].includes(p.type)&&BABYLON.Vector3.Distance(p.root.getAbsolutePosition?.()||p.root.position,e.root.position)<1.8);
        if(cand.length){const p=cand[Math.floor(Math.random()*cand.length)];p.heldBy="extraNpc";p.loose=false;p.vel.set(0,0,0);p.ang.set(0,0,0);p.hitMeshes.forEach(m=>removeCollision(m));e.heldProp=p;e.heldPropTime=0;playImpactSound('pickup',.48);e.objectCooldown=.35;}
        else e.objectCooldown=1.1;
      }
