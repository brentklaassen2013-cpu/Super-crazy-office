...
(() => {
  ...
  function playNpcVoice(line) {
    ...
    try {
      ...
      a.volume = .95;
      a.playbackRate = .96 + Math.random() * .08;
      lastNpcVoice = a;
      a.play().catch(() => {});
    } catch (_) {}
  }
  ...
  // ---------- Player torso / floor support ----------
  // Visual torso only: shoulders -> hips. No long body hanging toward the floor.
  const PLAYER_TORSO_HEIGHT = .54;
  const PLAYER_TORSO_RADIUS = .23;
  const playerBody = BABYLON.MeshBuilder.CreateCapsule("playerTorso", {
    height: PLAYER_TORSO_HEIGHT,
    radius: PLAYER_TORSO_RADIUS,
    tessellation: 14
  }, scene);
  playerBody.material = MAT.accent;
  playerBody.visibility = .50;
  ...
  function updatePlayerBody() {
    ...
    const head = xrCamera.globalPosition;

    // Compact arm-locomotion torso: high chest to waist/high hips.
    const topY = head.y - .16;
    const bottomY = head.y - .70;

    let forward = xrCamera.getForwardRay(1).direction.clone();
    forward.y = 0;
    if (forward.lengthSquared() > .001) forward.normalize();
    ...
    playerBody.rotation.y = Math.atan2(forward.x, forward.z);
  }
  ...
  function spawnRagdoll(origin, launchDir, strength) {
    const pieces = [];
    const specs = [
      {shape:"box",   size:[.48,.70,.28], off:[0,.95,0]},
      {shape:"sphere",size:[.42],         off:[0,1.55,0]},
      {shape:"box",   size:[.18,.62,.18], off:[-.34,1.05,0]},
      {shape:"box",   size:[.18,.62,.18], off:[ .34,1.05,0]},
      {shape:"box",   size:[.20,.68,.20], off:[-.16,.34,0]},
      {shape:"box",   size:[.20,.68,.20], off:[ .16,.34,0]}
    ];
    for (const sp of specs) {
      let m;
      if (sp.shape==="sphere") {
        m = BABYLON.MeshBuilder.CreateSphere("ragPart",{diameter:sp.size[0],segments:10},scene);
      } else {
        m = BABYLON.MeshBuilder.CreateBox("ragPart",{width:sp.size[0],height:sp.size[1],depth:sp.size[2]},scene);
      }
      m.material = MAT.rag;
      m.position = origin.add(new BABYLON.Vector3(...sp.off));
      const scatter = new BABYLON.Vector3((Math.random()-.5)*1.5, Math.random()*1.4, (Math.random()-.5)*1.5);
      const vel = launchDir.scale(strength).add(scatter).add(new BABYLON.Vector3(0,2.2,0));
      const collisionRadius = sp.shape==="sphere"
        ? sp.size[0]*.5
        : Math.max(sp.size[0], sp.size[1], sp.size[2])*.50;

      pieces.push({
        mesh:m, vel, collisionRadius,
        spin:new BABYLON.Vector3((Math.random()-.5)*8,(Math.random()-.5)*8,(Math.random()-.5)*8),
        life:3.2
      });
    }
  ...
  function finishNpc(hitPos, swingDir, swingSpeed) {
    ...
    const launch = Math.min(9.5, 4.0 + swingSpeed*.55);
  ...
  function damageNpc(hitPos, swingDir, speed) {
    ...
    const dir = swingDir.lengthSquared()>.001 ? swingDir.normalize() :
                npc.root.position.subtract(hitPos).normalize();
    const knockStrength = Math.min(4.8, .7 + speed*.30);
    ...
    if (npc) {
      ...
      if (npc.alive) {
        ...
        if (xrCamera) {
          ...
          if (d <= 1.05 && npc.attackCooldown <= 0) {
            npc.attackCooldown = 1.05;
            npc.attackAnim = .30;
            playerHP = Math.max(0, playerHP - 10);
            pulse(hands.left, .58, 70);
            pulse(hands.right, .58, 70);
            npcSpeak(["Got you!", "Back off!"], true);
            updateHpText();

            if (playerHP <= 0) {
              updateHpText("YOU WERE KNOCKED OUT!");
              setTimeout(() => {
